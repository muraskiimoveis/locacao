  ...
  <tr>
    <td height="31" align="left" valign="top" background="images/fundo_degrade_titulos.jpg"><table border="0" cellpadding="0" cellspacing="5">
      <tr>
        ...
        <td class="style47"><!--IM&Oacute;VEIS PARA LOCA&Ccedil;&Atilde;O ANUAL--><? if($calendario<>'1'){ ?>TEMPORADA POR DATA<? }else{ ?>CALENDÁRIO NEUTRALIZADO<? } ?></td>
        </tr>
    </table></td>
...
/*
	session_register("dial");
	session_register("mesl");
	session_register("anol");
	session_register("dial1");
	session_register("mesl1");
	session_register("anol1");
*/	
	...
	*/
	...
	if($calendario=='1'){
	  	...
	  	if($finalidade=='1' || $finalidade=='8'){
			...
			m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade2 $query_averbada AND m.disp_rede='1' AND m.ref!='x' AND m.relacao_bens LIKE '%$relacao_bens%'
			ORDER BY t.t_nome, m.ref LIMIT $from, 10";
			...
			AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
			m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada AND m.ref!='x' AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'
			ORDER BY t.t_nome, m.ref LIMIT $from, 10";
		}
	}else{

		$query1 = "SELECT * FROM muraski m LEFT JOIN rebri_tipo t ON (m.tipo=t.t_cod)
        WHERE (m.tipo LIKE '$tipo1' OR m.tipo_secundario LIKE '%-".$tipo1."-%')
        AND m.ref LIKE '$ref'
		AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
		m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada AND (m.data_inicio >= '$anol-$mesl-$dial' AND m.data_fim <= '$anol1-$mesl1-$dial1' OR
		'$anol1-$mesl1-$dial1' BETWEEN m.data_inicio AND m.data_fim) AND m.ref!='x' AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'
		ORDER BY t.t_nome, m.ref LIMIT $from, 10";
	...
	#print $query1;
	$result1 = mysql_query($query1) or die ("Erro 271 - ".mysql_error());
...
?>
    ...
    <input type="hidden" name="dial" id="dial" value="<? if($_GET['dial']){ echo $_GET['dial']; }else{ echo $dial; } ?>">
    <input type="hidden" name="mesl" id="mesl" value="<? if($_GET['mesl']){ echo $_GET['mesl']; }else{ echo $mesl; } ?>">
    <input type="hidden" name="anol" id="anol" value="<? if($_GET['anol']){ echo $_GET['anol']; }else{ echo $anol; } ?>">
    <input type="hidden" name="dial1" id="dial1" value="<? if($_GET['dial1']){ echo $_GET['dial1']; }else{ echo $dial1; } ?>">
    <input type="hidden" name="mesl1" id="mesl1" value="<? if($_GET['mesl1']){ echo $_GET['mesl1']; }else{ echo $mesl1; } ?>">
    <input type="hidden" name="anol1" id="anol1" value="<? if($_GET['anol1']){ echo $_GET['anol1']; }else{ echo $anol1; } ?>">
...
<? if($calendario<>'1'){ ?>	
	Pesquisa de <?php print("$dial/$mesl/$anol"); ?> à <?php print("$dial1/$mesl1/$anol1"); ?>.
<? } ?>	
	</td>
  </tr>
<?
if($calendario<>'1'){
	...
	if($_GET['acao'])
	{
	  ...
	  $msgErro = "";
		...
		$busca = mysql_query($SQL) or die ("Erro 404 - ".mysql_error());
...
if($calendario<>'1'){
	...
	if($numrows1 > 0){
		...
		while($not1 = mysql_fetch_array($result1))
		{
		    ...
		    $buscaIM = mysql_query("SELECT im_cod, im_nome, nome_pasta FROM rebri_imobiliarias WHERE im_cod='".$not1['cod_imobiliaria']."'")  or die ("Erro 442 - ".mysql_error());
		...
		if($codi=='3'){
			...
			if ($dial != "" and $mesl != "" and $anol != "" and $dial1 != "" and $mesl1 != "" and $anol1 != "") {
				...
				$query4 = "SELECT * FROM locacao WHERE l_imovel='$not1[cod]' and l_n_contrato!=''
				AND ('$anol-$mesl-$dial' BETWEEN l_data_ent AND l_data_sai or  
				'$anol1-$mesl1-$dial1' BETWEEN l_data_ent AND l_data_sai) AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'"; 
				#print $query4;

				$result4 = mysql_query($query4) or die ("Erro 472 - ".mysql_error());
  ...
  <tr>
    <td colspan="2" align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		<?php print("$not1[t_nome]"); ?> -- <b>Ref.: <?php print("$not1[ref]"); ?> - <?php print $not1[titulo]; ?></b><!--/a-->         <? if($not1[observacoes2] <> '') { ?>
...
?>
			  <tr>
                ...
                <td align="left" class="style48"><strong>R$ <?php print("$valor2"); ?></strong></td>
                </tr>
<?
			  }
?>
...
?>
              <tr>
                ...
                <td align="left" class="style48"><strong>R$ <?php print("$carnaval"); ?></strong></td>
                </tr>
<?php
	          }
?>
...
?>
              <tr>
                ...
                <td align="left" class="style48"><strong>R$ <?php print("$anonovo"); ?></strong></td>
                </tr>
<?php
	          }
?>
...
?>
              <tr>
                ...
                <td align="left" class="style48"><strong><?php print("$not1[acomod]"); ?> pessoas</strong></td>
                </tr>
<?php
	          }
?>
              <tr>
                <td colspan="2" align="left" class="style48">*O valor pode ser alterado sem aviso pr&eacute;vio.</td>
                </tr>
<?
			  ## se tem end, mostra!
			  if($not1[end] <> ''){
...
?>
              <tr>
                <td colspan="2" align="left" class="style48"><a href="cadastro_anuncios.php?qtd=1&rel=R&cod=<?php print("$not1[cod]"); ?>&codi=<?=$codi; ?>&troca=S&url=<?php print("$url"); ?>&nome_veiculo=<?=$nome_veiculo ?>" class="style48"><b>Trocar veículo</b></a></td>
              </tr>
<?
		}else{
?>
...
?>             
                <!--tr>
                <td colspan="2" align="left" class="style48"><a href="p_rel_loc.php?cod=<?php print("$not1[cod]"); ?>" class=style48><b>Visualizar Relatório de Locações</b></a></td>
                </tr>
                  <tr>
                <td colspan="2" align="left" class="style48"><a href="p_extrato_locacao.php?cod_imovel=<?php print("$not1[cod]"); ?>" class=style48><b>Extrato de Locações</b></a></td>
                </tr-->
              <tr>
...
?>
              ...
              <tr>
                <td colspan="2" align="left"><table width="100%" border="0" cellspacing="5" cellpadding="5">
                  <tr>
<? if($codi == $_SESSION['cod_imobiliaria']){ ?>                  
...
if ($conta_vi == 0) {
?>
					<td align="center"><a href="add_prod.php?qtd=1&cod=<?php print("$not1[cod]"); ?>&codi=<?=$codi; ?>&controle=L&url=<?php print("$url"); ?>"><img src="images/icones/adicionar_a_lista.jpg" width="34" height="34" border="0" title="Adicionar à lista" /></a></td>
<?
} else {
?>
...
<? if($codi == $_SESSION['cod_imobiliaria']){ ?>                     
					...
					<?  if($calendario<>'1'){ ?>
					<td align="center"><a href="reserva_mes.php?cod=<?php print("$not1[cod]"); ?>&dial=<? echo($dial); ?>&mesl=<? echo($mesl); ?>&anol=<? echo($anol); ?>&dial1=<? echo($dial1); ?>&mesl1=<? echo($mesl1); ?>&anol1=<? echo($anol1); ?>"><img src="images/icones/reservar_imovel.jpg" width="34" height="34" border="0" title="Fazer reserva" /></a></td>
       				<? }
					   else
					   { ?>
...
<tr><td valign="top">
              ...
              if($calendario<>'1'){
					...
					$query3 = "select * from locacao where l_imovel='$not1[cod]' AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' and l_n_contrato!='' order by l_data_ent ";


					$result3 = mysql_query($query3) or die ("Erro 794 - ".mysql_error());
  ...
  ## se encontrou imoveis
	if($numrows1 > 0){
		...
		while($not1 = mysql_fetch_array($result1))
		{
		    ...
		    $buscaIM = mysql_query("SELECT im_cod, im_nome, nome_pasta FROM rebri_imobiliarias WHERE im_cod='".$not1['cod_imobiliaria']."'") or die ("Erro 875 - ".mysql_error());
		    while($linhaIM = mysql_fetch_array($buscaIM)){
		    	$codi = $linhaIM['im_cod'];
				$nomei = $linhaIM['im_nome'];
				$pastai = $linhaIM['nome_pasta'];
...
<? if($codi == $_SESSION['cod_imobiliaria']){ ?>                       
                    <td align="center"><a href="p_edit_imoveis.php?cod=<?php print("$not1[cod]"); ?>"><img src="images/icones/atualizar_imovel.jpg" width="34" height="34" border="0" title="Atualizar imóvel" /></a></td>                   
					<?  if($calendario<>'1'){ ?>
					<td align="center"><a href="reserva.php?cod=<?php print("$not1[cod]"); ?>&dial=<? echo($dial); ?>&mesl=<? echo($mesl); ?>&anol=<? echo($anol); ?>&dial1=<? echo($dial1); ?>&mesl1=<? echo($mesl1); ?>&anol1=<? echo($anol1); ?>"><img src="images/icones/reservar_imovel.jpg" width="34" height="34" border="0" title="Fazer reserva" /></a></td>
       				<? }
					   else
					   { ?>
					<td align="center"><img src="images/icones/reservar_imovel_apagada.jpg" width="34" height="34" border="0" title="Fazer reserva" /></td>  
...
<tr><td valign="top">
              ...
              if($calendario<>'1'){
					...
					$result3 = mysql_query($query3) or die ("Erro 1203 - ".mysql_error());
					$numrows3 = mysql_num_rows($result3);
					if($numrows3 > 0){
...
<?php
	...
	if($calendario=='1'){
	  ...
	  if($finalidade=='1' || $finalidade=='8'){
			...
			$query2 = "SELECT count(m.cod) as contador FROM muraski m INNER JOIN rebri_tipo t ON (m.tipo=t.t_cod) INNER JOIN rebri_imobiliarias i ON (m.cod_imobiliaria=i.im_cod) WHERE i.im_desativar='0' AND (m.tipo LIKE '$tipo1' OR m.tipo_secundario LIKE '%-".$tipo1."-%') AND m.ref LIKE '$ref'
			AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
			m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade2 $query_averbada AND m.disp_rede='1' AND m.ref!='x' AND m.relacao_bens LIKE '%$relacao_bens%'";

		}else{

			$query2 = "SELECT count(m.cod) as contador FROM muraski m INNER JOIN rebri_tipo t ON (m.tipo=t.t_cod) WHERE (m.tipo LIKE '$tipo1' OR m.tipo_secundario LIKE '%-".$tipo1."-%') AND m.ref LIKE '$ref'
			AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
			m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada AND m.ref!='x' AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
		}
	}else{

		$query2 = "SELECT count(m.cod) as contador  FROM muraski m INNER JOIN rebri_tipo t ON (m.tipo=t.t_cod)
        WHERE (m.tipo LIKE '$tipo1' OR m.tipo_secundario LIKE '%-".$tipo1."-%')
        AND m.ref LIKE '$ref'
		AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
		m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada AND (m.data_inicio >= '$anol-$mesl-$dial' AND m.data_fim <= '$anol1-$mesl1-$dial1' OR
	...
	$result2 = mysql_query($query2) or die ("Erro 1314 - ".mysql_error());
