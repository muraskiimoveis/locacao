...
if($lista == "")
	...
	if($list == ""){
	$query1 = "select * from clientes WHERE c_tipo='prestador' AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND c_prestador='jardineiro' order by c_nome limit $from, 30";
	}else{
		if($data_tipo != ""){
	$query1 = "select * from clientes where c_prestaodr='prestador' AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND c_prestador='jardineiro' AND
	($data_tipo BETWEEN '$ano-$mes-$dia' AND '$ano1-$mes1-$dia1') 
	order by c_nome limit $from, 30";
		}
		else
		{
	$query1 = "select * from clientes where c_tipo='prestador' AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND c_prestador='jardineiro' AND $campo like '%$c_nome%' order by c_nome limit $from, 30";
		}
	}
	//echo $query1;
...
<table border="0" cellpadding="1" cellspacing="1" width="95%">
	<form method="get" action="p_list_jardineiro.php">
		<input type="hidden" value="1" name="list">
      <input type="hidden" value="" name="lista">
      <tr height="50">
		<td colspan=5 class="style1" align="center"><b>Relação de Jardineiros</b></td>
	</tr>
    ...
    <tr align=center>
	      <td colspan=5 class=style1><br /><br /><div align="left"><span class="style7">Se o jardineiro n&atilde;o est&aacute; cadastrado <a href="p_insert_clientes.php?novo=S&url=<?=$REQUEST_URI ?>" class="style7"><b>clique aqui</b></a></span></div></td>
    </tr>
  </form>
...
<td class="style1">
<p align="center">
<input type="button" onClick="window.opener.document.form1.nome_jardineiro.value='<?php print("$not[c_nome]"); ?>'; window.opener.document.form1.co_jardineiro.value='<?php print("$not[c_cod]"); ?>'; window.opener.focus(); window.close();" class="campo3" value="Selecionar"></td>
<!--a href="#" onClick="window.opener.document.form1.nome_jardineiro.value='<?php print("$not[c_nome]"); ?>'; window.opener.document.form1.co_jardineiro.value='<?php print("$not[c_cod]"); ?>'; window.opener.focus(); window.close();" class="style1">Selecionar</a></td-->
<?php
	}
	
	if($list == ""){
	$query2 = "select count(c_cod) as contador from clientes WHERE c_tipo='prestador' AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND c_prestador='jardineiro'";
	}else{
		if($data_tipo != ""){
	$query2 = "select count(c_cod) as contador from clientes where c_tipo='prestador' AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND c_prestador='jardineiro' AND 
	($data_tipo BETWEEN '$ano-$mes-$dia' AND '$ano1-$mes1-$dia1')";
		}
		else
		{
	$query2 = "select count(c_cod) as contador from clientes where c_tipo='prestador' AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND c_prestador='jardineiro' AND $campo like '%$c_nome%'";
		}
	}
...
?>
  <tr class="fundoTabelaTitulo">
  	<td colspan="5" class="style1"><p align="center"><b>Foram encontrados <?php print("$not2[contador]"); ?> jardineiros<b></p></td>
  </tr>
  <tr>
