...
function quant_dias_alocar($dat1, $dat2){
  return round((strtotime($dat2) -
     strtotime($dat1)) / (24 * 60 * 60), 0);
}

if(isset($_POST['opcao_7dias'])){
  $opcao_7dias = $_POST['opcao_7dias'];
}elseif(isset($_POST['opcao_10dias'])){
  $opcao_10dias = $_POST['opcao_10dias'];
}elseif(isset($_POST['opcao_15dias'])){
  $opcao_15dias = $_POST['opcao_15dias'];
}elseif(isset($_GET['opcao_7dias'])){
  $opcao_7dias = $_GET['opcao_7dias'];
}elseif(isset($_GET['opcao_10dias'])){
  $opcao_10dias = $_GET['opcao_10dias'];
}elseif(isset($_GET['opcao_15dias'])){
  $opcao_15dias = $_GET['opcao_15dias'];
}

$sql_dias ="";
if($opcao_7dias == '1'){
  $sql_dias = "AND m.dias_plocacao LIKE '%7%'";
}elseif($opcao_10dias == '1'){
  $sql_dias = "AND m.dias_plocacao LIKE '%10%'";
}elseif($opcao_15dias == '1'){
  $sql_dias = "AND m.dias_plocacao LIKE '%15%'";
}
...
// Alterado aki para não pegar na seleção 01 dia antes do inicio e 01 dia depois do final da RESERVA a pedido da Sra Patricia
// 20/11/2012
...
*/	
	...
	*/
	...
	if($calendario=='1'){
		...
		m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada $sql_dias AND m.ref!='x' $sql_opcao AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'
		ORDER BY $ordenar LIMIT $from, 10";
	}else{
        $qry_locado = "SELECT distinct m.ref FROM muraski m, locacao l WHERE  m.cod = l.l_imovel AND
        ((l.l_data_ent BETWEEN '$ano_x-$mes_x-$dia_x' AND '$ano1_x-$mes1_x-$dia1_x')OR
        (l.l_data_sai BETWEEN '$ano_x-$mes_x-$dia_x'  AND '$ano1_x-$mes1_x-$dia1_x')OR
        (l.l_data_ent < '$ano_x-$mes_x-$dia_x' AND l.l_data_sai > '$ano1_x-$mes1_x-$dia1_x'))";
     	$res_locado = mysql_query($qry_locado);
		while($rows_locado = mysql_fetch_array($res_locado))
        {
          $imoveis_locado .= $rows_locado['ref']."','";
        }

/*
		$query1 = "SELECT * FROM muraski m LEFT JOIN rebri_tipo t ON (m.tipo=t.t_cod) WHERE
        (m.tipo LIKE '$tipo1' OR m.tipo_secundario LIKE '%-".$tipo1."-%') AND m.ref LIKE '$ref'
        AND m.ref NOT IN ('$imoveis_locado')
		AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
		m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada AND
        ((m.data_inicio >= '$ano-$mes-$dia' AND m.data_fim <= '$ano1-$mes1-$dia1') OR ('$ano1-$mes1-$dia1' BETWEEN m.data_inicio AND m.data_fim)) AND
        m.ref!='x' $sql_opcao AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'
		ORDER BY $ordenar LIMIT $from, 10";
*/
		$query1 = "SELECT * FROM muraski m LEFT JOIN rebri_tipo t ON (m.tipo=t.t_cod) WHERE
        (m.tipo LIKE '$tipo1' OR m.tipo_secundario LIKE '%-".$tipo1."-%') AND m.ref LIKE '$ref'
        AND m.ref NOT IN ('$imoveis_locado')
		AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
		m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada $sql_dias AND
        ('$ano-$mes-$dia' >= m.data_inicio AND '$ano1-$mes1-$dia1' <= m.data_fim ) AND
        m.ref!='x' $sql_opcao AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'
		ORDER BY $ordenar LIMIT $from, 10";
...
/*
        ((m.data_inicio BETWEEN '$ano-$mes-$dia' AND '$ano1-$mes1-$dia1')OR
        (m.data_fim BETWEEN '$ano-$mes-$dia'  AND '$ano1-$mes1-$dia1')OR
	...
	if($numrows1 > 0){
		...
		if($codi=='3'){
			...
			if ($dia != "" and $mes != "" and $ano != "" and $dia1 != "" and $mes1 != "" and $ano1 != "") {
				...
				$query4 = "SELECT * FROM locacao WHERE l_imovel='$not1[cod]'
				AND ((l_data_ent' >= '$ano-$mes-$dia' AND l_data_sai <= '$ano1-$mes1-$dia1') OR
...
*/
				...
				if($numrows4 == 0){
                    ...
                    if($not1[ref] != ''){
                      ...
                      if(($dias_7 == '7') and ($dias_alocar < 7)){
                        //echo ('<script language="javascript">alert("LOCAÇÃO mínima para este IMÓVEL 7 (sete) DIAS! Deseja reservar mesmo assim??");"</script>');
?>
                       <script language="javascript">alert("LOCAÇÃO mínima para este IMÓVEL 7 (sete) DIAS! Deseja reservar mesmo assim??");</script>
<?php
                      }elseif(($dias_10 == '10') and ($dias_alocar < 10)){
                        //echo ('<script language="javascript">alert("LOCAÇÃO mínima para este IMÓVEL 10 (dez) DIAS! Deseja reservar mesmo assim??");"</script>');
?>
                       <script language="javascript">alert("LOCAÇÃO mínima para este IMÓVEL 10 (dez) DIAS! Deseja reservar mesmo assim??");</script>
<?php
                      }elseif(($dias_15 == '15') and ($dias_alocar < 15)){
                        //echo ('<script language="javascript">alert("LOCAÇÃO mínima para este IMÓVEL 15 (dez) DIAS! Deseja reservar mesmo assim??");"</script>');
?>
...
<?php
					...
					$feriado = number_format($not1[feriado], 2, ',', '.');
...
?>
              <tr>
                <td class="style48" align="left">Feriado:</td>
                <td colspan="2" align="left" class="style48"><strong>R$ <?php print("$feriado"); ?></strong></td>
                </tr>
<?php
	          }
?>
...
?>
              ...
              <? } ?>
					...
					<?  if($calendario<>'1'){ ?>
					<td align="center"><a href="reserva.php?cod=<?php print("$not1[cod]"); ?>&dia=<? echo($dia); ?>&mes=<? echo($mes); ?>&ano=<? echo($ano); ?>&dia1=<? echo($dia1); ?>&mes1=<? echo($mes1); ?>&ano1=<? echo($ano1); ?>&passagem=1"><img src="images/icones/reservar_imovel.jpg" width="34" height="34" border="0" title="Fazer reserva" /></a></td>
       				<? }
					   else
					   { ?>
...
<?php
    ...
    if($calendario=='1'){ 
		...
		AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
		m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada $sql_dias
		AND m.ref!='x' $sql_opcao AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
	}else{
        $qry_locado2 = "SELECT distinct m.ref FROM muraski m, locacao l WHERE  m.cod = l.l_imovel AND
        ((l.l_data_ent BETWEEN '$ano-$mes-$dia' AND '$ano1-$mes1-$dia1')OR
        (l.l_data_sai BETWEEN '$ano-$mes-$dia'  AND '$ano1-$mes1-$dia1')OR
        (l.l_data_ent < '$ano-$mes-$dia' AND l.l_data_sai > '$ano1-$mes1-$dia1'))";
     	$res_locado2 = mysql_query($qry_locado2);
	    //$rows_locado = mysql_fetch_array($res_locado);
		while($rows_locado2 = mysql_fetch_array($res_locado2))
        {
          $imoveis_locado2 .= $rows_locado2['ref']."','";
        }

		$query2 = "select count(m.cod) as contador
		from muraski m LEFT JOIN rebri_tipo t ON (m.tipo=t.t_cod) WHERE
        (m.tipo LIKE '$tipo1' OR m.tipo_secundario LIKE '%-".$tipo1."-%') AND m.ref LIKE '$ref' 
        AND m.ref NOT IN ('$imoveis_locado2')
		AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
		m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada $sql_dias
		AND ('$ano-$mes-$dia' >= m.data_inicio AND '$ano1-$mes1-$dia1' <= m.data_fim )
        AND m.ref!='x' $sql_opcao AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
...
/*
		...
		AND m.n_quartos $comp1 '$n_quartos' AND m.valor $comp2 '$valor' $dist_mar1 AND
		m.acomod $comp3 '$acomod' AND m.tipo_logradouro like '%$tipo_logradouro%' AND m.end LIKE '%$end%' AND m.numero LIKE '%$numero_end%' AND m.cep LIKE '%$cep%' $bairro $caracteristica $query_estado $query_cidade $query_finalidade $query_averbada
		AND ((m.data_inicio >= '$ano-$mes-$dia' AND m.data_fim <= '$ano1-$mes1-$dia1') OR ('$ano1-$mes1-$dia1' BETWEEN m.data_inicio AND m.data_fim))
        AND m.ref!='x' $sql_opcao AND m.relacao_bens LIKE '%$relacao_bens%' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
