...
/*
echo "Mostrar Quantos ==> ".$quantos."<br>";

echo "Mostrar SQL 2 ==> ".$sql0."<br>";

while ($not0 = mysql_fetch_assoc($crs)) {
echo "Mostrar ref ==> ".$not0[a_ref]."<br>";
}
die();
*/
...
while ($not0 = mysql_fetch_assoc($rs0)) {
echo "Mostrar ref ==> ".$not0[a_ref]."<br>";
}
die();
*/
...
/*
while ($not1 = mysql_fetch_assoc($rs1)) {
echo "Mostrar acao ==> ".$not1[a_acao]."<br>";
}
die();
*/
...
$sql2 .= " AND ((a_data = '".date("Y-m-d",$d)."' AND a_hora >= '".date("H:i:s",$d)."')";
$sql2 .= " or (a_data > '".date("Y-m-d",$d)."')) AND (a_acao like '%Imagem%' or ";
$sql2 .= " a_acao = 'Inserir Imóvel')  and cod_imobiliaria = '$cod_imobiliaria' ORDER BY a_id DESC LIMIT 1";

#echo "Mostrar SQL ==> ".$sql2;
#die();
...
#      echo "</muraski>";
	...
	if($imagens == 1) {
		...
		$vendab = "../imobiliarias/".$nome_pasta."/venda_big/";
		$loca = "../imobiliarias/".$nome_pasta."/locacao/";
		$locap = "../imobiliarias/".$nome_pasta."/locacao_peq/";
		$locab = "../imobiliarias/".$nome_pasta."/locacao_big/";
		if ($not[finalidade] <= 7) {
			$pasta = $venda;
			$pastap = $vendap;
			$pastab = $vendab;
