...
<table border="0" cellpadding="1" cellspacing="1" bgcolor="#<?php print("$cor1"); ?>">
	<form method="get" action="p_list_clientes.php">
     ...
     <tr bgcolor="#<?php print("$cor1"); ?>" align=center>
	      <td colspan=5 class=style1><div align="left"><span class="style7">Se o cliente n&atilde;o est&aacute; cadastrado <a href="p_insert_clientes.php?novo=S&url=<?=$REQUEST_URI ?>" class="style7">clique aqui</a></span></div></td>
    </tr>
  </form>
<tr>
...
<?php
	...
	$query2 = "select count(c_cod) as contador from clientes where cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
		...
		if($data_tipo != ""){
	$query2 = "select count(c_cod) as contador from clientes where 
	($data_tipo BETWEEN '$ano-$mes-$dia' AND '$ano1-$mes1-$dia1') and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
		}
		else
		{
	$query2 = "select count(c_cod) as contador from clientes where $campo like '%$c_nome%' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
		}
	}
