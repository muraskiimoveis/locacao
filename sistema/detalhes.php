...
<table border="0" cellpadding="1" cellspacing="1" width="75%">
  <tr height="50">
    <td>
      <p align="center" class="style48"><b>Detalhes do Im�vel</b> - <a href="detalhes_impressao.php?cod=<?=$cod ?>&codi=<?=$codi ?>&pastai=<?=$pastai ?>&nomei=<?=$nomei ?>" class="style48" target="_blank">Clique para Imprimir</a></p>
    </td>
  </tr>
<?php
   ...
   if ($ncoordenadas <> "") {
	...
	if(($not1[local] == "Caiob�") or ($not1[local] == "Matinhos")){
...
//    }else{$arquivo_existe = file_exists($pasta . $nome_foto1);}
    ...
    $arquivo_existe = file_exists($pasta . $nome_foto1);
...
<input type=button onClick="rotate(x+1);" value=">>" title="Pr�xima Foto" class=campo3>
<input type=button onClick="rotate(this.form.slide.length-1);" value="&gt;&gt;|" title="�ltima Foto" class=campo3>
...
//        }else{$arquivo_foto = file_exists($pasta . $nome_foto);}
        $arquivo_foto = file_exists($pasta . $nome_foto);
...
//            }else{$arquivo_foto2 = file_exists($pasta . $nome_foto2);}
            ...
            $arquivo_foto2 = file_exists($pasta . $nome_foto2);
...
//    }else{$arquivo_fotop = file_exists($pasta . $nome_fotop);}
    $arquivo_fotop = file_exists($pasta . $nome_fotop);
	//if (file_exists($pasta . $nome_fotop))
	if ($arquivo_fotop)
	{	
	/* retirado da imagem width="<?=$largura ?>" */
?>
<img border="0" width="900" height="430" alt="" src="<?php print($pasta.$nome_fotop."?datafo=$datafo&horafo=$horafo"); ?>" name="show">
<?
    } 
?>
    </td></tr>
...
<b>Su�tes:</b></td><td width="80%" class="style48"><?php print("$not1[suites]"); ?></td></tr>
<?php
	}
?><trr>
...
<tr><td width="20%" class="style48">
<b>Localiza��o:</b></td><td width="80%" class="style48"><?php print("$not1[ci_nome]"); ?> - <?php print("$not1[e_uf]"); ?></td></tr>
<?php
	if($not1[piscina] > 0){
?><tr><td width="20%" class="style48">
...
?>
					<tr><td width="20%" class="style48">
					<b>Acomoda��es:</b></td><td class="style48">
...
?>
			  <tr><td width="20%" class="style48">
					<b>Endere�o:</b></td><td class="style48">
...
?>
					<tr><td width="20%" class="style48">
					<b>Dist�ncia do mar:</b></td><td class="style48">
...
Especifica��o:</td><td width="410" class="style48"><?php print("$not1[especificacao]"); ?></td></tr>
<?php
//	}
?><tr-->
<?
...
<b>Descri��o do Im�vel:</b><p><?php print $descricao; ?></p>
...
?>
			  <tr>
               	<td colspan="2" class="style48"><b>V�deo:</b></td>
              </tr> 
              <tr>
...
if ($ncoordenadas <> "") {
   ...
   //echo "Mostra AKI ".$SRV_ORIGEM;
   
   
?>
  <iframe src="http://<? echo $SRV_ORIGEM; ?>/intranet/sistema/mapa_site_n.php?<?=$url_site?>" allowtransparency="true" width="780" height="680" scrolling="no" frameborder="0" marginheight="0" marginwidth="0"></iframe>
<?
} else {
?>
  <iframe src="http://<? echo $SRV_ORIGEM; ?>/intranet/sistema/mapa.php?ori=<?=$origem_mapa ?>&end=<?php echo ucwords(strtolower($endereco_mapa))."|".$not1[ref]."|".tirar_acentos($not1[t_nome])."|".$not1['bairro']."|".ucwords(strtolower($endereco_balao))."|".$not1['end_aproximado']."|".$not1['exibir_endereco']."|".$codi."|".$_SESSION['cod_imobiliaria']."|sistema";  ?>" allowtransparency="true" width="762" height="672" scrolling="no" frameborder="0" marginheight="0" marginwidth="0"></iframe>
<?
}
?>
  <? //}else{ ?>
...
function valida()
{
  ...
  {
    alert("Por favor, digite o M�s de Entrada");
  ...
  {
    alert("Por favor, digite o Dia de Sa�da");
    document.form1.dia1.focus();
    return (false);
  }
  if (document.form1.mes1.value == "")
  {
    alert("Por favor, digite o M�s de Sa�da");
    document.form1.mes1.focus();
    return (false);
  }
  if (document.form1.ano1.value == "")
  {
    alert("Por favor, digite o Ano de Sa�da");
...
<tr><td colspan=4 class="style48">
<a href="detalhes2.php?cod=<?php print("$not1[cod]"); ?>&mes=<?php print("$mes"); ?>&ano=<?php print("$ano"); ?>&codi=<?php print("$codi"); ?>&nomei=<?php print("$nomei"); ?>&pastai=<?php print("$pastai"); ?>" class="style48"><b>Ver detalhes com calend�rio</b></a><br>
</td></tr>
<?php
