...
<?php
	...
	$query1 = "select * from contas c INNER JOIN clientes cl ON (c.co_cliente=cl.c_cod) where c.co_locacao='$l_cod' and c.co_tipo='Despesas' and c.co_cat='Pagar' and c.co_cod!='$co_cod2' and c.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' order by c.co_data";
	//echo $query11;
...
<tr>
	<td colspan="6" class="style1"><b>Despesas do imóvel Ref.:</b> <?php print("$ref"); ?> - <?php print("$titulo"); ?></td>
</tr>
<tr>
<td width=325 bgcolor="#<?php print("$cor1"); ?>" align="left" class=style1><b>Prestador.</td>
<td width=142 bgcolor="#<?php print("$cor1"); ?>" align="left" class=style1><b>Descri&ccedil;&atilde;o</td>
<td width="168" bgcolor="#<?php print("$cor1"); ?>" class=style1><b>Valor</td>
<td width="142" bgcolor="#<?php print("$cor1"); ?>" class=style1><b>Cobrado</td>
<td width="94" bgcolor="#<?php print("$cor1"); ?>" class=style1><b>Data</td>
<td width="153" bgcolor="#<?php print("$cor1"); ?>" class=style1><b></td>
</tr>
<?php
	$i = 1;
	/*if($B1 == "Excluir"){
	$total_desp = $valor_mostrado;
	}
	else
	{
		$total_desp = 0;
	...
	*/
	$query101 = "select * from contas c INNER JOIN clientes cl ON (c.co_cliente=cl.c_cod) where c.co_locacao='$l_cod' and c.co_tipo='Despesas' and c.co_cat='Receber' and c.co_cod!='$co_cod2' and c.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' order by c.co_data";
	$result101 = mysql_query($query101);
	while($not101 = mysql_fetch_array($result101))
	{
	  $total_desp = $not101[co_valor] + $total_desp;  
	}
...
<td align="left" bgcolor="<?php print("$fundo"); ?>" class="style1">R$ <?php print("$not1[co_valor]"); ?></td>
<td align="left" bgcolor="<?php print("$fundo"); ?>" class="style1">
<? 
	$query10 = "select * from contas c INNER JOIN clientes cl ON (c.co_cliente=cl.c_cod) where c.co_locacao='$l_cod' and c.co_tipo='Despesas' and c.co_desc='$not1[co_desc]' and c.co_cat='Receber' and c.co_cod!='$co_cod2' and c.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' order by c.co_data";
	$result10 = mysql_query($query10);
	while($not10 = mysql_fetch_array($result10))
	{
		
?>
<input type="hidden" name="co_cod_prop" value="<?php print("$not10[co_cod]"); ?>"> 
<input type="hidden" name="valor_mostrado" value="<?php print("$not10[co_valor]"); ?>"> 
R$ <?php print("$not10[co_valor]"); ?>
<?
 	} 
?>
</td>
<td align="left" bgcolor="<?php print("$fundo"); ?>" class="style1"><?php print(formataDataDoBd($not1[co_data])); ?></td>
<td width="153" align="left" bgcolor="<?php print("$fundo"); ?>" class="style1">
...
<?php
	...
	if($B1 == "Excluir"){
			...
			//if($co_tipo=='Receber'){  		
				...
				$total_desp2 = $total_desp - $valor_mostrado;
				...
				$query = "delete from contas where co_cod = '$co_cod_prop' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
				$result = mysql_query($query) or die("Não foi possível apagar suas informações. $query");
				...
				$query7= "insert into contas (cod_imobiliaria, co_cliente, co_cat, co_imovel, co_desc, co_tipo, co_data, co_status, co_valor
				, co_locacao, co_usuario, co_forma, co_data_status, co_usuario_status) 
				values('".$_SESSION['cod_imobiliaria']."','$cliente20', 'Pagar', '$cod', '$co_desc', 'Locação', current_date, 'pendente', '$valor_mostrado'
...
<!--tr>
  <td class=style1 colspan=6>Legenda:<br>
<span class="style6">Azul - Despesas do propriet&aacute;rio</span><br>
<span class="style7">Vermelho - Despesas do prestador</span></td>
</tr-->
<tr>
