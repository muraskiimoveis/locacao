...
<div align="center">
 ...
 Preencha os campos do seu interesse e clique em pesquisar.</p></td>
    ...
    <tr class="fundoTabela">
      ...
      <td width="70%" class=style1> <select name="finalidade" class="campo" onChange="form1.action='p_pesq_ven.php';form1.acaob.value='1';form1.acaoci.value='1';form1.submit();">
		 <?php
        
		$bfinalidade = mysql_query("select f_cod, f_nome FROM finalidade WHERE f_nome LIKE 'Vend%' ORDER BY f_nome ASC");
...
?>
   ...
   <?
    ...
    <tr class="fundoTabela">
      ...
      <td class="style1">
        <input name="opcao_neutra" id="opcao_neutra" type="radio" value="1" <? echo "CHECKED"; ?> OnClick="seleciona_opcao_neutra()">
        ...
        <input name="opcao_com" id="opcao_com" type="radio" value="0" OnClick="seleciona_opcao_com()">
...
Exemplo: 80000000 </td>
    ...
    <tr class="fundoTabela">
	  ...
	  <select name="im_estado" class="campo" onChange="form1.action='p_pesq_ven.php';form1.acaoci.value='1';form1.submit();">
        ...
        if($_SESSION['cod_estadoi'] && empty($_GET['im_estado'])){          
			...
			$bestados = mysql_query("SELECT e.e_cod, e.e_uf FROM rebri_estados e INNER JOIN muraski m ON (m.uf=e.e_cod) WHERE m.disp_rede='1' AND (m.finalidade='2' OR m.finalidade='3' OR m.finalidade='4' OR m.finalidade='5' OR m.finalidade='6' OR m.finalidade='7') AND m.ref!='x' GROUP BY e.e_uf ORDER BY e.e_uf ASC");
 			while($linha = mysql_fetch_array($bestados)){
 		  	...
 		  	if($_GET['finalidade']<>'1'){
          		$query_cod_imo = "m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND ";
		  	}else{
			    $query_cod_imo = "m.disp_rede='1' AND ";
			}
		  	
		  	if($_GET['finalidade']=='7' || $_GET['finalidade']=='1'){
			    $query_finalidades = " (m.finalidade='2' OR m.finalidade='3' OR m.finalidade='4' OR m.finalidade='5' OR m.finalidade='6' OR m.finalidade='7') AND ";
			}else{
			  	$query_finalidades = " m.finalidade='".$_GET['finalidade']."' AND ";
			}
 		  
	        $bestados = mysql_query("SELECT e.e_cod, e.e_uf FROM rebri_estados e INNER JOIN muraski m ON (m.uf=e.e_cod) WHERE $query_cod_imo $query_finalidades m.ref!='x' GROUP BY e.e_uf ORDER BY e.e_uf ASC");
 			while($linha = mysql_fetch_array($bestados)){
				if($linha[e_cod]==$estado){
			   		echo('<option value="'.$linha[e_cod].'" SELECTED>'.$linha['e_uf'].'</option>');
				}else{
    ...
    <tr class="fundoTabela">
	  ...
	  <select name="local" class="campo" onChange="form1.action='p_pesq_ven.php';form1.acaob.value='1';form1.acaoci.value='1';form1.submit();">
       ...
       <?
 		...
 		if($_GET['acaoci']=='1'){
 		  ...
 		  if($_GET['finalidade']<>'1'){
	        ...
	        $bcidades = mysql_query("SELECT c.ci_cod, c.ci_nome, m.disponibilizar, m.local FROM rebri_cidades c INNER JOIN muraski m ON (m.local=c.ci_cod) WHERE $query_cod_imo $query_finalidades c.ci_estado='".$estado."' AND m.ref!='x' GROUP BY c.ci_nome ORDER BY c.ci_nome ASC");
			while($linha = mysql_fetch_array($bcidades)){
				if($linha[ci_cod]==$_GET['local']){
		...
		elseif($_SESSION['cod_cidadei'] && empty($_GET['local'])){
			...
			//$bcidades = mysql_query("SELECT c.ci_cod, c.ci_nome FROM rebri_cidades c INNER JOIN muraski m ON (m.local=c.ci_cod) WHERE  m.disp_rede='1' AND (m.finalidade='2' OR m.finalidade='3' OR m.finalidade='4' OR m.finalidade='5' OR m.finalidade='6' OR m.finalidade='7') AND c.ci_estado='".$estado."' AND m.ref!='x' GROUP BY c.ci_nome ORDER BY c.ci_nome ASC");
            $bcidades = mysql_query("SELECT c.ci_cod, c.ci_nome FROM rebri_cidades c INNER JOIN muraski m ON (m.local=c.ci_cod) WHERE  m.finalidade='2' AND c.ci_estado='".$estado."' AND m.ref!='x' GROUP BY c.ci_nome ORDER BY c.ci_nome ASC");
			while($linha = mysql_fetch_array($bcidades)){
                /*
