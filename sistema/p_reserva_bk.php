...
<tr>
    <td  colspan="3" align="center"><a href="javascript:history.back()" class="style1">Voltar</a></td>
 </tr>
<?php
	#}//Termina while 7
	}//Termina numrows 7
	...
	for ($j = 1; $j <= $qtd_parc; $j++) {
			...
			if($qtd_parc==1)
			{
				...
				for($i3 = 1; $i3 <= $contador; $i3++){
					...
					//parcela 1
					$query90 = "insert into contas (cod_imobiliaria, co_cliente, co_cat, co_imovel, co_desc, co_tipo, co_data, co_status, co_valor, co_locacao, co_usuario, co_forma, co_data_status, co_fixar) values('".$_SESSION['cod_imobiliaria']."','".$proprietario[$i3-1]."', 'Pagar', '$cod', '$l_pagto', 'Locação', '$anop1-$mesp1-$diap1', 'pendente', '$valores', '$l_cod', '$valid_user', 'Depósito', $data_status, '$co_fixar')";
					$result90 = mysql_query($query90) or die("Não foi possível atualizar suas informações." . $query90);

					//parcela 2
					$query100 = "insert into contas (cod_imobiliaria, co_cliente, co_cat, co_imovel, co_desc, co_tipo, co_data, co_status, co_valor, co_locacao, co_usuario, co_forma, co_data_status, co_fixar) values('".$_SESSION['cod_imobiliaria']."', '".$proprietario[$i3-1]."', 'Pagar', '$cod', '$l_pagto', 'Locação', '$anop2-$mesp2-$diap2', 'pendente', '$valores', '$l_cod', '$valid_user', 'Depósito', $data_status, '$co_fixar')";
			...
			else
			{
				...
				$dia4 = substr ($data_nova, 0, 2);
			  	...
			  	for($i3 = 1; $i3 <= $contador; $i3++){
			    	...
			    	$query10 = "insert into contas (cod_imobiliaria, co_cliente, co_cat, co_imovel, co_desc, co_tipo, co_data, co_status, co_valor, co_locacao, co_usuario, co_forma, co_data_status, co_fixar) values('".$_SESSION['cod_imobiliaria']."','".$proprietario[$i3-1]."', 'Pagar', '$cod', '$l_pagto', 'Locação', '$ano4-$mes4-$dia4', 'pendente', '$valores', '$l_cod', '$valid_user', 'Depósito', $data_status, '$co_fixar')";
...
<a href="#" onClick="NewWindow('','uprelatorio','800','600','yes');form2.target='uprelatorio';form2.action='ficha_inquilinos2.php?cod=<?php print("$cod"); ?>&codloc=<?php print("$codloc"); ?>&l_cod=<?php print("$l_cod"); ?>';form2.submit();" class="style1">
