	...
	if((retornaTrecho($registroAtual, 14, 1)=='U') || (retornaTrecho($registroAtual, 14, 1)=='T')){
		if(retornaTrecho($registroAtual, 14, 1)=='T'){
			...
			if($VlrTitulo == retornaTrecho($registroAtual, 93, 15)){
				...
				//$insere_nrdoc .= "update contas set co_status = 'ok', co_data_status = '".$dta_pgto."', co_conciliado = 'S' where co_boleto = '".$boleto;
				$insere_nrdoc .= "' and co_status = 'pendente' and co_forma = 'Boleto'";
