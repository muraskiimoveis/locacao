...
if($lista == "")
	...
	$query1 = "select * from clientes WHERE cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' order by c_nome limit $from, 30";
		...
		if($data_tipo != ""){
	$query1 = "select * from clientes where c_tipo='fiador' AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND
	($data_tipo BETWEEN '$ano-$mes-$dia' AND '$ano1-$mes1-$dia1') 
	order by c_nome limit $from, 30";
		}
		else
		{
	$query1 = "select * from clientes where cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND $campo like '%$c_nome%' order by c_nome 
	limit $from, 30";
		}
	}
	//echo $query1;
...
<table border="0" cellpadding="1" cellspacing="1" bgcolor="#<?php print("$cor1"); ?>">
	<form method="get" action="p_list_fiador.php">
		<input type="hidden" value="1" name="list">
      <input type="hidden" value="" name="lista">
      <input type="hidden" id="id" name="id" value="<?=$_GET['id'] ?>">
      <tr>
		<td bgcolor="#<?=$cor3 ?>" colspan=5 class="style1" align="center"><b>Relação de Fiadores</b></td>
	</tr>
    ...
    <tr bgcolor="#<?php print("$cor1"); ?>" align=center>
	      <td colspan=5 class=style1><div align="left"><span class="style7">Se o fiador n&atilde;o est&aacute; cadastrado <a href="p_insert_clientes.php?novo=S&url=<?=$REQUEST_URI ?>" class="style7">clique aqui</a></span></div></td>
    </tr>
  </form>
<tr>
...
<td bgcolor="#<?php print("$fundo"); ?>" class="style1">
<p align="center">
<input type="button" onClick="window.opener.document.formulario.fiador_<?php echo($id); ?>.value='<?php print("$not[c_cod]"); ?>';window.opener.document.formulario.nome_fiador_<?php echo($id); ?>.value='<?php print("$not[c_nome]"); ?>';window.opener.focus(); window.close();" class="campo3" value="Selecionar"></td>
<!--a href="#" onClick="window.opener.document.formulario.fiador.value='<?php print("$not[c_cod]"); ?>';window.opener.document.formulario.nome_fiador.value='<?php print("$not[c_nome]"); ?>';window.opener.focus(); window.close();" class="style1">Selecionar</a></td-->
<?php
	}
