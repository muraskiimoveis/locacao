...
// Calculo de dias para contrato e renova��o
...
function google(coord) {
   if (coord != 'inicio') {
      NewWindow('http://<? echo $ORIGEM; ?>/intranet/sistema/exemplo-api.php?ori=muraski&coord='+coord, 'janela', 800, 700, 'yes');
   } else {
      NewWindow('http://<? echo $ORIGEM; ?>/intranet/sistema/exemplo-api.php?ori=muraski&coord=inicio', 'janela', 800, 700, 'yes');
   }
}
...
function radio_button(valor,renova){
  if(renova == '1'){<?php $renova_simnao_new = '1'; ?>}
  if(renova == '0'){<?php $renova_simnao_new = '0'; ?>}
  document.getElementById('form1').action='p_edit_imoveis.php?cod=<?php echo($cod); ?>&edit=editar&m=1&renova_simnao=<?php echo($renova_simnao_new); ?>';
  document.getElementById('form1').acao.value=valor;
  document.getElementById('form1').submit();
}

function seleciona_renova_nao(){
    if(document.getElementById('renova_sim').checked){
      document.getElementById('renova_nao').checked = true;
      document.getElementById('renova_nao').value = '1';
      document.getElementById('renova_sim').checked = false;
	  document.getElementById('renova_sim').value = '0';
	  <?php $renova_simnao_new = '0'; ?>
	}else if(document.getElementById('renova_nao').checked){
      document.getElementById('renova_sim').checked = false;
	  document.getElementById('renova_sim').value = '0';
      <?php $renova_simnao_new = '0'; ?>
    }
	/*alert(<?php echo $renova_simnao_new; ?>);*/
    radio_button('2',<?php echo($renova_simnao_new); ?>);
}

function seleciona_renova_sim(){
    if(document.getElementById('renova_nao').checked){
      document.getElementById('renova_sim').checked = true;
      document.getElementById('renova_sim').value = '1';
      document.getElementById('renova_nao').checked = false;
	  document.getElementById('renova_nao').value = '0';
	  <?php $renova_simnao_new = '1'; ?>
	}else if(document.getElementById('renova_sim').checked){
      document.getElementById('renova_nao').checked = false;
	  document.getElementById('renova_nao').value = '0';
	  <?php $renova_simnao_new = '1'; ?>
    }
...
{
       if(confirm("Tem certeza que deseja ativar esse im�vel para venda?"))
...
function valida()
{
  ...
  {
    alert("Por favor, digite o M�s desejado");
...
if ($_GET['codigo']) {
	...
	$result4 = mysql_query($query4) or die("N�o foi poss�vel atualizar suas informa��es. $query4");
	$query40 = "update vendas set v_status='I' where v_imovel='$cod' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
	$result40 = mysql_query($query40) or die("N�o foi poss�vel atualizar suas informa��es. $query40");
	$query50 = "update sinal_venda set s_status='I' where cod_imovel='$cod' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
	$result50 = mysql_query($query50) or die("N�o foi poss�vel atualizar suas informa��es. $query50");
	$query60 = "update propostas set p_status='I' where cod_imovel='$cod' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
	$result60 = mysql_query($query60) or die("N�o foi poss�vel atualizar suas informa��es. $query60");
	print('<script language="javascript">alert("Im�vel ativado com sucesso!");document.location.href="p_edit_imoveis.php?cod='.$cod.'&edit=editar";</script>');
...
if ($_POST['apaga_imovel_definitivamente']=='1') {
    ...
    if((Trim($_SESSION['u_email']) != 'claudir@muraski.com') and ($_SESSION['u_cod'] != '21')){
     ...
     echo ('<script language="javascript">alert("E-mail e/ou Senha do Usu�rio com status N�O PODE EXCLUIR IMOVEL DEFINITIVAMENTE");document.location.href="'.$_SERVER['REQUEST_URI'].';"</script>');
     //include("p_imoveis.php");
     exit;
    }

  	$B1 = "Apagar Im�vel Definitivamente";
   ...
   if($numrows4 > 0){
	...
	if($contador_venda > 0){
		$msg2 .= "- Esse im�vel n�o pode ser exclu�do pois est� vinculado a uma venda<BR>\n";
      $confirm_exc = "n";
   	}
   
	if($contador_locacao > 0){
		$msg2 .= "- Esse im�vel n�o pode ser exclu�do pois est� vinculado a uma loca��o<BR>\n";
      $confirm_exc = "n";
   	}
   	
   	if($contador_contas > 0){
		$msg2 .= "- Esse im�vel n�o pode ser exclu�do pois est� vinculado a uma conta<BR>\n";
      $confirm_exc = "n";
   	}

   	if ($confirm_exc == "n") {
 
 		$msg = "<div align=\"center\" class=\"style7\"><b>Exclus�o n�o realizada</b><br>".$msg2."</div>";
   ...
   $hora = date("H:i:s");
	...
	//Insere o usu�rio que esta fazendo a atualiza��o do imovel
   $insere = mysql_query("INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_imovel, a_ref, a_acao, a_data, a_hora) VALUES ('".$_SESSION['cod_imobiliaria']."','".$u_cod."','".$_POST['cod']."','".$_POST['ref']."','".$B1."','".$data."','".$hora."')") or die ("Erro 87 - ".mysql_error());
    ...
    /*
	//Deleta o registro mais antigo da tabela, s� deixa apenas as 10 ultimas atualiza��es
   ...
   */
	...
	$msg = "<div align=\"center\" class=\"style7\">Voc� apagou definitivamente o im�vel Ref.: ".$_POST['ref']."</div>";


   	$query = "delete from muraski where cod = '".$_POST['cod']."' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
	$result = mysql_query($query) or die("N�o foi poss�vel apagar suas informa��es.");
...
if ($_POST['apaga_imovel']=='1') {
	...
	*/
	...
	$msg = "<div align=\"center\" class=\"style7\">Voc� apagou o im�vel Ref.: ".$_POST['ref']."</div>";
...
if ($_POST['altera']=="1") {
	...
	$feriado = str_replace(".", "", $_POST['feriado']);
   ...
   $numero = count($_POST['bairro']);
	...
	if($disp_rede=='0'){
	  $comissao_parceria = '';
		...
		if($totalperc > 100){
	 		...
	 		$msg =  "<div align=\"center\" class=\"style7\">A soma total dos percentuais dos propriet�rios � maior que 100!</div>";
		}
		elseif($totalperc < 100)
		{
		   $msg =  "<div align=\"center\" class=\"style7\">A soma total dos percentuais dos propriet�rios n�o pode ser inferior a 100!</div>";	
		}
		else
		{
	...
	$datai_renova = "$anor-$mesr-$diar";
	$dataf_renova = "$anor1-$mesr1-$diar1";
	if(isset($_POST[renova_simnao]) and ($_POST[renova_simnao]=='1')){
		$renova_simnao_new = '1';
		//$montasim = 'Mostra renova_sim ==> '.$renova_simnao_new;
		//echo '<script> javascript:alert("'.$montasim.'"); </script>';
	}elseif(isset($_POST[renova_simnao]) and ($_POST[renova_simnao]=='0')){
		$renova_simnao_new = '0';
		//$montanao = 'Mostra renova_nao ==> '.$renova_simnao_new;
		//echo '<script> javascript:alert("'.$montanao.'"); </script>';
	}

/*
	$montaatualiza = 'Mostra renova_simnao_new ==> '.$renova_simnao_new.
	' \nMostra renova_simnao => '.$not2[renova_simnao].
	' \nMostra POST[renova_sim] ==> '.$_POST[renova_sim].
	' \nMostra POST[renova_nao] ==> '.$_POST[renova_nao].
	' \nMostra datai_renova ==> '.$datai_renova.
	' \nMostra dataf_renova ==> '.$dataf_renova;

	echo '<script> javascript:alert("'.$montaatualiza.'"); </script>';	
*/	
	...
	, data_fim='$data_fim', comissao='$comissao', dias='$dias', dias_plocacao='".$dias_plocacao1."', contrato='$contrato'
	, datai_renova='$datai_renova', dataf_renova='$dataf_renova', renova_simnao='$renova_simnao_new', opcao_simnao='$opcao_simnao_new'
	, carnaval='$carnaval', anonovo='$anonovo', feriado='$feriado', coordenadas = '".$coordenadas."', posx='$posx', posy='$posy', tv='$tv'
   ...
   , exibir_endereco='$exibir_endereco', observacoes2='$observacoes2', observacoes3='$observacoes3', ncoordenadas='$ncoordenadas', calendario='$calendario'
	...
	$result = mysql_query($query) or die("N�o foi poss�vel atualizar suas informa��es.");
	...
	$msg = "<div align=\"center\" class=\"style7\">Voc� atualizou o im�vel Ref.: ".$ref."</div>";
	include("p_imoveis.php");
	exit;
...
<?php
   ...
   if($not2[dist_mar]=='' && $not2[dist_tipo]=='') {
	...
	if(isset($_POST['renova_sim']) and ($_POST['renova_sim']=='1')){
		$renova_simnao_new = '1';
	}elseif(isset($_POST['renova_nao']) and ($_POST['renova_nao']=='1')){
		$renova_simnao_new = '0';
	}elseif((!isset($_POST['renova_sim'])) or  (!isset($_POST['renova_nao']))){
		$renova_simnao_new = $not2[renova_simnao];
	}
	
	
	$anor = substr ($not2[datai_renova], 0, 4);
	$mesr = substr($not2[datai_renova], 5, 2 );
	$diar = substr ($not2[datai_renova], 8, 2 );
//
	$anor1 = substr ($not2[dataf_renova], 0, 4);
	$mesr1 = substr($not2[dataf_renova], 5, 2 );
	$diar1 = substr ($not2[dataf_renova], 8, 2 );
...
var msgErro = '';
		...
		{
			alert( "Por favor, selecione o campo Tipo do im�vel." );
		...
		else
		{
		  document.form1.tipo1.style.backgroundColor = '<?=$cor1 ?>';
       	...
       	else
	   	{
			...
			{
		  		alert( "N�o pode haver espa�o nem caractere especial no campo Refer�ncia." );
				document.form1.ref.focus();
				document.form1.ref.style.backgroundColor = '<?=$cor_erro ?>';
				return false;
			}
  		...
  		{
				alert("Por favor, selecione o n� de propriet�rios");
		...
		<? for($i = 1; $i <= $_POST['cont']; $i++){ ?>
  			...
  			{
    			alert("Por favor, selecione o Propriet�rio <?=$i ?>");
  		...
  		<? } ?>	  		
	   	if(document.form1.finalidade.value=="1" || document.form1.finalidade.value=="2" || document.form1.finalidade.value=="3" || document.form1.finalidade.value=="4" || document.form1.finalidade.value=="5" || document.form1.finalidade.value=="6" || document.form1.finalidade.value=="7")
	   	{
	     	if(document.form1.dias.value.length==0)
  			{
    			alert( "Por favor, preencha o campo Dias �teis." );
		...
		else
		{
		  document.form1.diae.style.backgroundColor = '<?=$cor1 ?>';
       	...
       	{
    		alert( "Por favor, preencha o campo M�s Inicial de Contrato de." );
		...
		else
		{
		  document.form1.diae1.style.backgroundColor = '<?=$cor1 ?>';
       	...
       	{
         	alert( "Por favor, preencha o campo M�s Final de Contrato de." );
       	...
       	if(document.form1.finalidade.value=="15" || document.form1.finalidade.value=="16" || document.form1.finalidade.value=="17")
  	   	{
  			...
  			{
    			alert( "Por favor, preencha o campo Di�ria." );
		...
		else
		{
  		...
  		{
    		alert( "Por favor, preencha o campo Comiss�o Imobili�ria." );
		...
		else
		{
  		...
  		if(document.form1.finalidade.value=="15" || document.form1.finalidade.value=="16" || document.form1.finalidade.value=="17")
  		{
  			...
  			{
    			alert( "Por favor, preencha o campo Di�ria m�nima." );
			...
			else
			{
  			...
  			{
    			alert( "Por favor, preencha o campo Di�ria m�xima." );
		...
		else
		{
  		...
  		{
			alert( "Por favor, selecione o campo Localiza��o." );
		...
		else
		{
  		...
  		if(document.form1.finalidade.value=="15" || document.form1.finalidade.value=="16" || document.form1.finalidade.value=="17")
  		{
  			...
  			{
    			alert( "Por favor, preencha o campo Acomoda��es." );
		...
		else
		{
  		...
  		if(document.form1.disp_rede.value=="1")
  		{
    		if(document.form1.comissao_parceria.value.length==0)
  			{
    			alert( "Por favor, preencha o campo Comiss�o oferecida p/ parceria." );
  			...
  			if(document.form1.comissao_parceria.value=="diferenciado")
			{
  				...
  				{
    				alert( "Por favor, preencha o campo Comiss�o." );
		...
		if(document.form1.video.value!="")
  		{
  			...
  			{
    			alert( "Por favor, selecion o campo Origem do V�deo." );
    	...
    	return true;
	   	...
	   	if(document.form1.diab.value.length==0)
  	   	{
  	    	alert( "Por favor, preencha o campo Dia Rela��o de Bens." );
		...
		else
		{
		  document.form1.diab.style.backgroundColor = '<?=$cor1 ?>';
       	...
       	{
    		alert( "Por favor, preencha o campo M�s Rela��o de Bens." );
		...
		else
		{
		  document.form1.mesb.style.backgroundColor = '<?=$cor1 ?>';
       	...
       	{
         	alert( "Por favor, preencha o campo Ano Rela��o de Bens." );
...
<div align="center">
  ...
  <tr height="50"><td colspan=2 class="style1" align="center"><b>Editar ou Apagar Im�vel</b> <?=$msg ?></td></tr>
    ...
    <tr class="fundoTabela">
      <td colspan="2" class=style1>
	  	   <fieldset><legend><b>Tipo Secund�rio:</b></legend>
	  <?
	...
	<tr class="fundoTabela">
      <td width="35%" class=style1><b>Refer�ncia:</b></td>
      <td width="65%" class=style1> <input type="text" class="campo" name="ref" id="ref" size="10" maxlength="10" value="<?php print("$not2[ref]"); ?>" onKeyUp="return autoTab(this, 10, event);"> <a href="javascript:;" onClick="NewWindow('p_ref.php?menu=N', 'janela', 750, 500, 'yes')" class="style1"><b>Ver refer�ncias: usadas e dispon�veis</b></a></td>
   </tr>
	<!--tr class="fundoTabela">
      <td width="35%" class=style1><b>Propriet�rio:</b></td>
    ...
    <tr class="fundoTabela">
      <td width="35%" class=style1><b>Propriet�rio: N� de campos: </b><select name="cont" id="cont" class="campo" onChange="form1.action='p_edit_imoveis.php?cod=<?php echo($cod); ?>&edit=editar&m=1';form1.submit();">
		  <option value="">Selecione</option>
          ...
          </select><br><span class="style7">Obs: Para substituir um propriet�rio que j� existe por outro basta clicar no bot�o "Selecionar" e selecionar outro propriet�rio</span></td>
...
<?
            if($_GET['contador']){
		     ...
		     for($i3 = 1; $i3 <= $contador; $i3++){
			   ...
			   if($contador<>'1'){
			   		echo("
						<a href=\"p_edit_imoveis.php?cod=".$cod."&edit=editar&contador=".$contador."&cont=".$cont."\" class=\"style1\" title=\"Para remover o propriet�rio deve clicar em Remover e depois no bot�o Atualizar Im�vel\"> Remover</a><br>
			   		");
			   }
...
?>         
	...
	<tr class="fundoTabela">
      <td width="35%" class=style1><b>Im�vel constru�do em:</b></td>
      <td width="65%" class=style1>
	...
	<tr class="fundoTabela">
      <td width="35%" class=style1><b>Condom�nio:</b></td>
      <td width="65%" class=style1>
    ...
    <!--tr class="fundoTabela">
      <td class=style1><b>Comiss&atilde;o Propriet�rio:</b></td>
...
?>
    ...
    <tr class="fundoTabela">
		...
		<input type="button" value="Calcular data final" name="calcular" id="calcular" class="campo3" onClick="radio_button('1','9');">
		<b>�</b> 
<?php
	...
	if($_POST['acao']=='1'){
		...
		if(($diae1<>'00') && ($diae1<>'')){
			$datai_new = $diae1."/".$mese1."/".$anoe1;
			$diasu_new = $_POST['dias'];
			$data_final_new = somar_dias_uteis($datai_new, $diasu_new);
			list($diar10, $mesr10, $anor10) = explode('/', $data_final_new);
			$anor = $anoe1;
			$mesr = $mese1;
			$diar = $diae1;
		}else{
...
?>
	...
	<tr class="fundoTabela">
		<td width="35%" class=style1><b>Renova��o Automatica.? &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b>  
		<input name="renova_sim" id="renova_sim" type="radio" value="1"  <? if($renova_simnao_new =='1'){ echo "CHECKED"; } ?> OnClick="seleciona_renova_sim();">
			[ Sim ]
<?php 
			if(verificaFuncao('USER_RENOVA_NAO')){ 
?>	
        <input name="renova_nao" id="renova_nao" type="radio" value="0" <? if($renova_simnao_new =='0'){ echo "CHECKED"; }  ?> OnClick="seleciona_renova_nao();">
			[ N&atilde;o ]
<?php 
			} else {
?>
        <input name="renova_nao" id="renova_nao" type="radio" value="0" <? if($renova_simnao_new =='0'){ echo "CHECKED"; }  ?> OnClick="alert('Usuario N�o Autorizado!!');">
			[ N&atilde;o ]
<?php
			} 
?>
...
<?php
	...
	if($_POST['acao']=='2'){
		...
		if(!isset($anor10)){
			$anor10 = substr ($not2[dataf_renova], 0, 4);
			$mesr10 = substr($not2[dataf_renova], 5, 2 );
			$diar10 = substr ($not2[dataf_renova], 8, 2 );
		}
	}

	if(($_POST['diae1']<>'00') && ($_POST['diae1']<>'') && ($_POST['acao']=='2')){
		$datai_new = $_POST['diae1']."/".$_POST['mese1']."/".$_POST['anoe1'];
		$diasu_new = $_POST['dias'];
		$data_final_new = somar_dias_uteis($datai_new, $diasu_new);
		list($diar10, $mesr10, $anor10) = explode('/', $data_final_new);
		$anor = $_POST['anoe1'];
...
/*
$montaalerta = 'Mostra renova_simnao_new ==> '.$renova_simnao_new.
' \nMostra renova_simnao => '.$not2[renova_simnao].
' \nMostra POST[renova_sim] ==> '.$_POST[renova_sim].
' \nMostra POST[renova_nao] ==> '.$_POST[renova_nao].
' \nMostra datai_new ==> '.$datai_new.
' \nMostra diasu_new ==> '.$diasu_new.
' \nMostra data_final_new ==> '.$data_final_new;

echo '<script> javascript:alert("'.$montaalerta.'"); </script>';
*/

	if($renova_simnao_new=='1'){
	
		if((substr ($not2[datai_renova], 8, 2 )<>'00') && (substr ($not2[datai_renova], 8, 2 )<>'')){
			$datai_new = substr ($not2[datai_renova], 8, 2 )."/".substr ($not2[datai_renova], 5, 2 )."/".substr ($not2[datai_renova], 0, 4 );
			$diasu_new = $not2[dias];
			$data_final_new = somar_dias_uteis($datai_new, $diasu_new);
			list($diar10, $mesr10, $anor10) = explode('/', $data_final_new);
			$anor = substr ($not2[datai_renova], 0, 4);
			...
			//echo '<script> javascript:alert("Passou Aki "); </script>';	
			...
			if(($diae1<>'00') && ($diae1<>'')){
				...
				$diasu_new = $not2[dias];
				$data_final_new = somar_dias_uteis($datai_new, $diasu_new);
				list($diar1, $mesr1, $anor1) = explode('/', $data_final_new);
				$anor = $anoe1;
				$mesr = $mese1;
				$diar = $diae1;
			}
...
*/		
		...
		if(($diasu_new > 0 ) && ($data_final_new<>'')){

		
?>	
  
			<td width="65%" class=style1> 
			<input readonly="readonly" type="text" name="diar" id="diar" value="<?php if($_POST['diar']){ print($_POST['diar']); }else{ print($diar); } ?>" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input readonly="readonly" type="text" name="mesr" id="mesr" value="<?php if($_POST['mesr']){ print($_POST['mesr']); }else{ print($mesr); } ?>" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input readonly="readonly" type="text" name="anor" id="anor" value="<?php if($_POST['anor']){ print($_POST['anor']); }else{ print($anor); } ?>" size="4" class="campo" maxlenght="4" onKeyUp="return autoTab(this, 4, event);">
			<input readonly="readonly" type="button" value="Calcular data final" name="calc_renova" id="calc_renova" class="campo3" onClick="">
			<b>�</b> 
			<input readonly="readonly" type="text" name="diar1" id="diar1" value="<?php if($diar1){ print($diar1); }else{ print($diar10); } ?>" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input readonly="readonly" type="text" name="mesr1" id="mesr1" value="<?php if($mesr1){ print($mesr1); }else{ print($mesr10); } ?>" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input readonly="readonly" type="text" name="anor1" id="anor1" size="4" class="campo" maxlenght="4" onKeyUp="return autoTab(this, 4, event);" value="<?php if($anor1){ print($anor1); }else{ print($anor10); } ?>"> Ex.: 
			10/10/1910
			</td>
			
<?php
		} else { 
			echo "<script> javascript:alert(\"Faltam informa��es para Renova��o Autom�tica !!\"); </script>";
?>
...
?>
		...
		<td width="65%" class=style1> 
		<input type="text" name="diar" id="diar" value="<?php if($_POST['diar']){ print($_POST['diar']); }else{ print($diar); } ?>" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input type="text" name="mesr" id="mesr" value="<?php if($_POST['mesr']){ print($_POST['mesr']); }else{ print($mesr); } ?>" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input type="text" name="anor" id="anor" value="<?php if($_POST['anor']){ print($_POST['anor']); }else{ print($anor); } ?>" size="4" class="campo" maxlenght="4" onKeyUp="return autoTab(this, 4, event);">
		<input type="button" value="Calcular data final" name="calc_renova" id="calc_renova" class="campo3" onClick="form1.action='p_edit_imoveis.php?cod=<?php echo($cod); ?>&edit=editar&m=1';form1.acao.value='2';form1.submit();" >
		<b>�</b> 
		<input type="text" name="diar1" id="diar1" value="<?php if($diar1){ print($diar1); }else{ print($diar10); } ?>" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input type="text" name="mesr1" id="mesr1" value="<?php if($mesr1){ print($mesr1); }else{ print($mesr10); } ?>" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input type="text" name="anor1" id="anor1" size="4" class="campo" maxlenght="4" onKeyUp="return autoTab(this, 4, event);" value="<?php if($anor1){ print($anor1); }else{ print($anor10); } ?>"> Ex.: 
		10/10/1910
		</td>
		</tr>
...
?>
    <tr class="fundoTabela">
      <td class=style1><b>Situa��o da Loca��o : </b></td>
      <td class="style1">
...
<!-- AKI COME�A -->
    ...
    <tr class="fundoTabela">
      <td class=style1><b>Pacotes de Loca��o : </b></td>
      <td class="style1">
	  <?
...
?>
	<tr class="fundoTabelaDestaque">
   		<td colspan=2 class="style1" height="25px">
   			<p align="left"><b><a href="p_rel_loc_mes.php?cod=<?php print("$not2[cod]"); ?>" class=style1>Visualizar Relat�rio de Loca��es</a></b></p>
   		</td>
  	</tr>
<? }elseif(($not2[finalidade] == "15") or ($not2[finalidade] == "16") or ($not2[finalidade] == "17")){ ?>
	<tr class="fundoTabelaDestaque">
  		<td colspan=2 class="style1" height="25px">
    		<p align="left"><b><a href="p_rel_loc.php?cod=<?php print("$not2[cod]"); ?>" class=style1>Visualizar Relat�rio de Loca��es</a></b></p>
    	</td>
  	</tr>	
<? } ?>
  	<tr class="fundoTabelaDestaque">
  		<td colspan=2 class="style1" height="25px">
    		<p align="left"><b><a href="javascript:;" onClick="NewWindow('p_extrato_locacao.php?cod_imovel=<?php print("$not2[cod]"); ?>', 'janela', 750, 500, 'yes')" class="style1"><b>Extrato de Loca��es</b></a></b></p>
    	</td>
  	</tr>
<?php
	}
...
?>
   ...
   <tr class="fundoTabela">
      <td class=style1><b>Situa��o da Venda : </b></td>
      <td class="style1">
        <input name="opcao_sem" id="opcao_sem" type="radio" value="0" <? if($not2[opcao_simnao]=='0'){ echo "CHECKED"; } ?> OnClick="seleciona_opcao_sem()">
        SEM Op&ccedil;&atilde;o assinada
        <input name="opcao_com" id="opcao_com" type="radio" value="1" <? if($not2[opcao_simnao]=='1'){ echo "CHECKED"; } ?> OnClick="seleciona_opcao_com()">
...
function <?php print("$imovel3"); ?>()
{
    ...
    <tr class="fundoTabelaDestaque">
    	<td colspan=2 class="style1" height="25px">
    		<p align="left"><b><a href="javascript:<?php print("$imovel3"); ?>();" class=style1>Imprimir Renova��o</a></b></p>
    	</td>
    </tr>
<?php
	//}
...
function <?php print("$imovel2"); ?>()
{
	...
	<tr class="fundoTabelaDestaque">
		<td colspan=2 class="style1" height="25px">
    		<p align="left"><b><a href="p_rel_int.php?cod=<?php print("$not2[cod]"); ?>" class=style1>Visualizar Relat�rio de Interessados</a></b></p>
    	</td>
    </tr>
	<tr class="fundoTabelaDestaque">
		<td colspan=2 class="style1" height="25px">
...
function <?php print("imovel_bens"); ?>()
{
    ...
    <tr class="fundoTabelaDestaque">
    	<td colspan=2 class="style1" height="25px">
    		<p align="left"><b><a href="javascript:<?php print("imovel_bens"); ?>();" class=style1>Imprimir Rela��o de bens</a></b></p>
    	</td>
    </tr>
    <tr class="fundoTabelaDestaque">
    	<td colspan=2 class="style1" height="25px">
    ...
    <!--tr class="fundoTabelaDestaque">
    	<td colspan=2 class="style1" height="25px">
      		<p align="left"><b><a href="javascript:;" onClick="NewWindow('cadastro_anuncios2.php?cod=<?php echo("$not2[cod]"); ?>&rel=S', 'janela', 750, 500, 'yes')" class="style1">Editar Exporta��es</a></b></p>
      	</td>
    </tr-->
    <tr class="fundoTabelaDestaque">
    	<td colspan=2 class="style1" height="25px">
      		<p align="left"><b><a href="p_rel_anuncios.php?cod=<?php print("$not2[cod]"); ?>" class=style1>Visualizar Relat&oacute;rio de Exporta��es </a></b></p>
      	</td>
    </tr>
<?
...
?>
    <tr class="fundoTabelaDestaque">
		<td colspan=2 class="style1" height="25px">
			<p align="left"><b><a href="javascript:;" onClick="NewWindow('despesas.php?cod=<?php echo("$not2[cod]"); ?>', 'janela', 750, 500, 'yes')" class="style1">Despesas do Im�vel</a></b></p>
		</td>
    </tr>
<? } ?>
	<!--tr class="fundoTabela">
      <td colspan=2 class="style1"><b><a href="#" onClick="NewWindow('', 'uprelatorio', 750, 500, 'yes');form1.target='uprelatorio';form1.action='cadastro_servicos.php?cod=<?php echo("$not2[cod]"); ?>';form1.submit();" class=style1>Servi�os de Cobran�a</a></b></td>
    </tr-->
    <tr class="fundoTabela">
		<td class=style1>
...
?>
    ...
    <tr class="fundoTabela">
      <td class=style1><b>Valor Feriado:</b></td>
      <td class=style1><input type="text" name="feriado" size="10" class="campo" value="<?php print(number_format($not2[feriado], 2, ',', '.')); ?>" onKeydown="Formata(this,20,event,2)"></td>
    </tr>
    <tr class="fundoTabela">
		...
		<tr>
      		<td class="style1" height="25px">
      			<select name="tipo_logradouro" id="tipo_logradouro" class="campo">
					...
					<option value="�rea" <? if($not2['tipo_logradouro']=='�rea'){ echo "SELECTED"; } ?>>�rea</option>
					...
					<option value="Ch�cara" <? if($not2['tipo_logradouro']=='Ch�cara'){ echo "SELECTED"; } ?>>Ch�cara</option>
					<option value="Col�nia" <? if($not2['tipo_logradouro']=='Col�nia'){ echo "SELECTED"; } ?>>Col�nia</option>
					<option value="Condom�nio" <? if($not2['tipo_logradouro']=='Condom�nio'){ echo "SELECTED"; } ?>>Condom�nio</option>
					...
					<option value="Esta��o" <? if($not2['tipo_logradouro']=='Esta��o'){ echo "SELECTED"; } ?>>Esta��o</option>
					...
					<option value="N�cleo" <? if($not2['tipo_logradouro']=='N�cleo'){ echo "SELECTED"; } ?>>N�cleo</option>
    				...
    				<option value="P�tio" <? if($not2['tipo_logradouro']=='P�tio'){ echo "SELECTED"; } ?>>P�tio</option>
					<option value="Pra�a" <? if($not2['tipo_logradouro']=='Pra�a'){ echo "SELECTED"; } ?>>Pra�a</option>
					...
					<option value="S�tio" <? if($not2['tipo_logradouro']=='S�tio'){ echo "SELECTED"; } ?>>S�tio</option>
...
?>
      	<tr>
      		<td style="text-align: justify; padding-right: 5px">
      			<span class="style7">Para que o mapa funcione corretamente sem precisar preencher as coordenadas do Google Maps no campo "Coordenadas" � preciso o tipo de logradouro + endere�o completo, o campo n&uacute;mero e o CEP preenchidos. (Exemplo: Avenida 29 de Abril 601 CEP: 80000000)</span>
      		</td>
      	<tr>
<? /**/ ?>
      </table>
...
<? if ($ncoordenadas == "") { ?>
         <input type="button" value="Localizar Endere�o" name="B1" class="campo3" Onclick="google('inicio');"> </td>
<? } else {
      $tcoord = str_replace("(", "", $ncoordenadas);
      $tcoord = str_replace(")", "", $tcoord);
?>
         <input type="button" value="Localizar Endere�o" name="B1" class="campo3" Onclick="google('<?=urlencode($tcoord)?>');"> </td>
<? } ?>
    </tr>
...
<? }else{ ?>
    <tr class="fundoTabela">
	  ...
	  * Mapa da InfoCenter somente colocar as coordenadas do im�vel separados por / (barra). Ex: 4146/1872</td>
...
<? /**/ ?>
    <tr class="fundoTabela">
      <td class=style1><b>Exibir endere�o no site?</b></td>
      <td class="style1"><input name="exibir_endereco" type="checkbox" id="exibir_endereco" value="1" <? if($not2[exibir_endereco]=='1'){ print "CHECKED"; } ?>>
        N�o exibir<br><span class="style7">Obs: Se este campo estiver marcado o �cone no mapa do Google e o endere�o do im�vel n�o aparecer� no site.</span></td>
    </tr>
...
Exemplo: 80000000 <br>
           <span class="style7">Caso o Google n�o exiba corretamente a localiza��o no mapa preencha o endere�o com a localiza��o correta do mapa. (Exemplo: Avenida 29 de Abril 601 CEP: 80000000)</span></td>
     </tr>
...
?>
      ...
      <td class="style1"><input name="end_aproximado" type="checkbox" id="end_aproximado" value="1" <? if($not2[end_aproximado]=='1'){ print "CHECKED"; } ?>>
        Sim<br><span class="style7">Obs: Se este campo estiver marcado o �cone no mapa do Google n�o aparecer�.</span></td>
    </tr>
<?
/**/
?>
 ...
 ?>
	...
	<tr class="fundoTabela">
      <td width="35%" class="style1"><b>Localiza��o:</b></td>
   ...
   <tr class="fundoTabela">
      <td width="35%" class=style1><b>T�tulo:</b> </td>
    ...
    <tr class="fundoTabela">
      <td width="35%" class=style1><b>Descri��o:</b></td>
...
?>    
    <tr class="fundoTabela">
      ...
      <td class=style1><select class="campo" name="permuta" id="permuta">
          ...
          <option <? if($not2[permuta]=='N�o'){ print "SELECTED";  } ?>>N&atilde;o</option>
      </select></td>
    </tr>
    <tr class="fundoTabela">
    ...
    <tr class="fundoTabela">
      ...
      <td class="style1"><input name="opcao" id="opcao1" type="radio" value="1" <? if($opcao=='1'){ echo "CHECKED"; } ?> OnClick="LimpaCampo();">
        ...
        <select size="1" name="dist_mar1" id="dist_mar1" class="campo" <? if($opcao=='1'){ ?> disabled="disabled" style="background:#D6D6D6;" <? }?>>
          <option value="frente para a ba�a" <? if($dist_mar1=='frente para a ba�a'){ echo "SELECTED"; } ?>>frente para a ba&iacute;a</option>
...
<? } ?>
   ...
   <tr class="fundoTabela">
    ...
    <tr class="fundoTabela">
      <td class=style1><b>Cart�rio/Oficio:</b></td>
     ...
     if($numrowsv1 > 0){
       while($row3 = mysql_fetch_array($queryv1)){
         if($row3['disponibilizar']=='1'){
         	$mensagem = "<span class=\"style7\"><br><b>Contrato est� vencido!</b><br>Para que esse im�vel volte a aparecer no site � preciso alterar a data final do contrato para uma data maior ou igual a data atual!</span>";
         }
       }  
     }
     */
...
?>
	<tr class="fundoTabela">
      <td class=style1><b>Destaque no site da Rebri:</b><br /><span class="style7">Obs: Se o im�vel n�o possuir foto e estiver como destaque, ele n�o ser� exibido no site da Rebri.</span></td>
...
?>
    <? if($_SESSION['im_site_padrao']=='S'){ ?>
	 <tr class="fundoTabela">
      <td class=style1><b>Destaque no site padr�o:</b><br />
        <span class="style7">Obs: Se o im�vel n�o possuir foto e estiver como destaque, ele n�o ser� exibido no site padr&atilde;o.</span></td>
	 ...
	 <tr class="fundoTabela">
      <td class=style1><b>Lan�amento:</b><br /></td>
	...
	?>
	<tr class="fundoTabela">
      <td class=style1><b>Exibir calend�rio no site da rebri e no site padr�o:</b></td>
...
?>
    <tr class="fundoTabela">
      <td class=style1><b>Observa��es 01 (Vis�veis atrav�s do �cone de observa��es do sistema):</b></td>
    ...
    <tr class="fundoTabela">
      <td class=style1><b>Observa��es 02 (Complemento do titulo do im�vel vis�vel no portal, site e sistema):</b></td>
    ...
    <tr class="fundoTabela">
      <td class=style1><b>Observa��es 03 (Complemento de informa��es adicionais sobre o im�vel sempre vis�vel, apenas no sistema):</b></td>
...
<!--
   Colocar data da rela��o de bens AQUI
    <tr class="fundoTabela">
      <td class=style1><b>Data da Rela��o de bens:</b></td>
      <td class=style1>
...
?>
    <tr class="fundoTabela">
      <td width="35%" class=style1><b>Data da Rela��o de bens:</b></td>
      <td width="65%" class=style1>
    ...
    <tr class="fundoTabela">
      <td class=style1><b>C�digo V�deo:</b><br>
    ...
    <tr class="fundoTabela">
      <td class=style1><b>Origem do V�deo:</b></td>
    ...
    <!--tr class="fundoTabela">
      <td width="23%" class=style1><b>Localiza��o:</b></td>
    ...
    <option>S�o Jos� dos Pinhais</option>
        </select></td>
    </tr-->
    <!--tr class="fundoTabela">
      <td width="23%" class=style1><b>Tipo Divulga��o:</b></td>
    ...
    <tr class="fundoTabela">
      <td colspan="2"><table width="100%" border="0" cellpadding="1" cellspacing="1">
        <tr>
          <td colspan="4" class="style1"><b>&Uacute;ltimas tr�s atualiza&ccedil;&otilde;es</b></td>
        </tr>
        <tr class="fundoTabelaTitulo">
          <td width="15%" class="style1"><b>Data</b></td>
          <td width="17%" class="style1"><b>Hora</b></td>
		  <td width="32%" class="style1"><b>A��o</b></td>
          <td width="36%" class="style1"><b>Usu�rio</b></td>
        </tr>
		<?
		$i = 0;
...
?>
    ...
    <tr class="fundoTabela">
      ...
      <input type="hidden" name="renova_simnao" id="renova_simnao" value="<? echo($renova_simnao_new); ?>">
      <input type="button" value="Atualizar Im�vel" class="campo3" name="B1" Onclick="VerificaCampo(<? echo($not2[cod]); ?>)">
      <input type="button" value="Apagar Im�vel" class=campo3 name="B1" onClick="javascript:confirmaExclusao2();">
