...
<div align="center">
  ...
  <table width="75%" border="0" cellpadding="1" cellspacing="1">
    <tr height="50">
      <td width="100%" colspan=2 class=style1><p align="center"><b>Imóveis p/ Locação <!--Mensal-->Anual</b><br>
...
<?php
	...
	unset($_SESSION['mesl']);
	unset($_SESSION['anol']);
	unset($_SESSION['dial1']);
	unset($_SESSION['mesl1']);
	unset($_SESSION['anol1']);
?>	
    <tr class="fundoTabela">
    ...
    <tr class="fundoTabela">
      ...
      <td width="70%" class=style1> <select name="finalidade" class="campo" onChange="form1.action='p_pesq_loc_mes.php';form1.acaob.value='1';form1.acaoci.value='1';form1.submit();">
      	 <?php
        
		$bfinalidade = mysql_query("select f_cod, f_nome FROM finalidade WHERE f_nome LIKE 'Locação_Anual%' ORDER BY f_cod ASC");
...
Exemplo: 80000000 </td>
    ...
    <tr class="fundoTabela">
	  ...
	  <select name="im_estado" class="campo" onChange="form1.action='p_pesq_loc_mes.php';form1.acaoci.value='1';form1.submit();">
        ...
        if($_SESSION['cod_estadoi'] && empty($_GET['im_estado'])){
			...
			$bestados = mysql_query("SELECT e.e_cod, e.e_uf FROM rebri_estados e INNER JOIN muraski m ON (m.uf=e.e_cod) WHERE m.disp_rede='1' AND (m.finalidade='9' OR m.finalidade='10' OR m.finalidade='11' OR m.finalidade='12' OR m.finalidade='13' OR m.finalidade='14') AND m.ref!='x' GROUP BY e.e_uf ORDER BY e.e_uf ASC");
 			while($linha = mysql_fetch_array($bestados)){
 		  	...
 		  	if($_GET['finalidade']<>'8'){
          		$query_cod_imo = "m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND ";
		  	}else{
		    	$query_cod_imo = "m.disp_rede='1' AND ";
		  	}
		  	
		  	if($_GET['finalidade']=='8' || $_GET['finalidade']=='14'){
			    $query_finalidades = " (m.finalidade='9' OR m.finalidade='10' OR m.finalidade='11' OR m.finalidade='12' OR m.finalidade='13' OR m.finalidade='14') AND ";
			}else{
    ...
    <tr class="fundoTabela">
	  ...
	  <select name="local" class="campo" onChange="form1.action='p_pesq_loc_mes.php';form1.acaob.value='1';form1.acaoci.value='1';form1.submit();">
       ...
       <?
 		...
 		if($_GET['acaoci']=='1'){
 		  ...
 		  if($_GET['finalidade']<>'8'){
			...
			$bcidades = mysql_query("SELECT c.ci_cod, c.ci_nome FROM rebri_cidades c INNER JOIN muraski m ON (m.local=c.ci_cod) WHERE  m.disp_rede='1' AND (m.finalidade='9' OR m.finalidade='10' OR m.finalidade='11' OR m.finalidade='12' OR m.finalidade='13' OR m.finalidade='14') AND c.ci_estado='".$estado."' AND m.ref!='x' GROUP BY c.ci_nome ORDER BY c.ci_nome ASC");
 			while($linha = mysql_fetch_array($bcidades)){
