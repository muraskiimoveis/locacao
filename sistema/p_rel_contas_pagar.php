...
$nome_cliente2 = $_GET['nome_cliente2'];
...
$html3 .='<page><table border="0" cellspacing="1" width="800">
  			<tr>
    			<td colspan="6" style="text-align:left;">';
					...
					{
						$html3 .='<img src="'.$caminho_logo.$logo_imob.'" border="0" width="100">';
					}
    ...
    <tr>
      <td colspan="6" style="font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;color: #666666;text-align:center;"><b>Relat&oacute;rio Contas a Pagar </b></td>
    </tr>';
...
$html3 .= '<tr>
    ...
    <tr>
      <td colspan="6" style="font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;color: #666666;"><b>Per&iacute;odo: </b> '.$data_inicial.' &agrave; '.$data_final.'<b><br>
      </b><b>Prop./Ind./Vend./Ang.: </b>'.$nome_cliente.'<br><b>Usuário: </b>'.$usuarios.'</td>
    </tr>';
  	      ...
  	      if(!empty($cliente)){
				...
				$busca = mysql_query("SELECT DATE_FORMAT(co.co_data, '%d/%m/%Y') AS data_conta, m.ref, m.tipo_logradouro, m.end, m.numero, co.co_valor, co.co_status, co.co_tipo, c.c_nome, u.u_nome FROM contas co LEFT JOIN muraski m ON (co.co_imovel=m.cod) LEFT JOIN clientes c ON (co.co_cliente=c.c_cod) LEFT JOIN usuarios u ON (co.co_cliente=u.u_cod) WHERE m.ref!='x' AND co.co_cliente='".$cliente."' $query_tipo $query_usuario $query_status AND co.co_cat='Pagar' AND co.co_data BETWEEN '$data1' AND '$data2' AND co.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' ORDER BY co_data ASC");
			...
			';
				...
				$busca = mysql_query("SELECT DATE_FORMAT(co.co_data, '%d/%m/%Y') AS data_conta, m.ref, m.tipo_logradouro, m.end, m.numero, co.co_valor, co.co_status, co.co_tipo, c.c_nome, u.u_nome FROM contas co LEFT JOIN muraski m ON (co.co_imovel=m.cod) LEFT JOIN clientes c ON (co.co_cliente=c.c_cod) LEFT JOIN usuarios u ON (co.co_cliente=u.u_cod) WHERE m.ref!='x' AND co.co_cliente='".$cliente2."' $query_tipo $query_usuario $query_status AND co.co_cat='Pagar' AND co.co_data BETWEEN '$data1' AND '$data2' AND co.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' ORDER BY co_data ASC");
				while($linha = mysql_fetch_array($busca)){
            ...
            $html3 .='
				<tr>
          			<td width="200" style="background:#EEEEEE;font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;padding: 4px;"><b>Imóvel</b></td>
                    <td width="200" style="background:#EEEEEE;font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;padding: 4px;"><b>Proprietário</b></td>
          			<td width="50" style="background:#EEEEEE;font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;padding: 4px;"><b>Data</b></td>
          			<td width="50" style="background:#EEEEEE;font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;padding: 4px;"><b>Tipo</b></td>
                    <td width="50" style="background:#EEEEEE;font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;padding: 4px;"><b>Valor</b></td>
                    <td width="50" style="background:#EEEEEE;font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;padding: 4px;"><b>Status</b></td>
				...
				$busca = mysql_query("SELECT DATE_FORMAT(co.co_data, '%d/%m/%Y') AS data_conta, m.ref, m.tipo_logradouro, m.end, m.numero, co.co_valor, co.co_status, co.co_tipo, co.co_tipo_user, c.c_nome, u.u_nome FROM contas co LEFT JOIN muraski m ON (co.co_imovel=m.cod) LEFT JOIN clientes c ON (co.co_cliente=c.c_cod) LEFT JOIN usuarios u ON (co.co_cliente=u.u_cod) WHERE m.ref!='x' AND co.co_cat='Pagar' AND co.co_data BETWEEN '$data1' AND '$data2' $query_tipo $query_usuario $query_status AND co.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' ORDER BY co_data ASC");
				while($linha = mysql_fetch_array($busca)){
           				
				if($linha['co_tipo_user']=='A' || $linha['co_tipo_user']=='V'){ 
				      $ncliente = $linha['u_nome'];
				 }else{
				      $ncliente = $linha['c_nome'];
				 }
				
					$html3 .='
						<tr>
          					<td width="200" style="font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;color: #666666;">'.$linha['ref'].' - '.$linha['tipo_logradouro'].' '.$linha['end'].', '.$linha['numero'].'</td>
                        	<td width="200" style="font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;color: #666666;">'.$ncliente.'</td>
                        	<td width="50" style="font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;color: #666666;">'.$linha['data_conta'].'</td>
                        	<td width="50" style="font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;color: #666666;">'.$linha['co_tipo'].'</td>
                        	<td width="50" style="font-family: Verdana,Arial, Helvetica, sans-serif;font-size: 10px;color: #666666;">'.number_format($linha['co_valor'],2,',','.').'</td>
				   ';
...
<form id="form1" name="form1" method="post" action="">
 <table width="75%" border="0" align="center" cellpadding="1" cellspacing="1">
	<tr height="50">
      <td width="100%" colspan=2 class="style1"><p align="center"><b>Relat&oacute;rio Contas a Pagar </b><br>
 Preencha o período e demais campos que você deseja visualizar o relatório e clique em pesquisar</p></td>
    ...
    <tr class="fundoTabela">
      <td width="20%" class="style1"><b>Propriet&aacute;rio/Indicador:</b></td>
    ...
    <tr class="fundoTabela">
      <td class="style1" colspan="2"><b>OU</b></td>
    </tr>
    <tr class="fundoTabela">
      <td width="20%" class="style1"><b>Angariador/Vendedor:</b></td>
      <td width="80%" class="style1"><input type="text" name="cliente2" size="5" class="campo2" value="<?php print($cliente2); ?>" readonly>
        <input type="text" name="nome_cliente2" size="60" class="campo" value='<?php print($nome_cliente2); ?>' readonly>
        <input type="button" id="selecionar3" name="selecionar3" value="Selecionar" class="campo3" onClick="NewWindow('p_list_ang_vend.php', 'janela', 750, 500, 'yes');"></td>
    </tr>
	<tr class="fundoTabela">
  ...
  <table width="75%" border="0" align="center" cellpadding="1" cellspacing="1">
    ...
    <tr>
      <td colspan="2" class="style1"><b>Prop./Ind./Vend./Ang.:</b> <? if(!empty($_POST['nome_cliente'])){ echo $_POST['nome_cliente'];  }elseif(!empty($_POST['nome_cliente2'])){ echo $_POST['nome_cliente2'];  }else{ echo "Todos"; } ?><br>
<?
	  if($_POST['busuario']<>'Todos'){       
      ...
      <td colspan="2"><table width="100%" border="0" cellpadding="1" cellspacing="1">
        <?
            ...
            echo('
				<tr class="fundoTabelaTitulo">
          		  <td width="40%" class="style1"><b>Imóvel</b></td>
                  <td width="20%" class="style1"><b>Prop./Ang./Ind./Vend.</b></td>
        ...
        <tr>
           <td aling="center" colspan="6"><form name="form3" id="form3" method="post" action="">
             <div align="center">
               <input type="submit" value="Exportar para PDF" name="exportar" id="exportar" class="campo3" onClick="form3.action='p_rel_contas_pagar.php?pdf=1&data_inicial=<?=$data_inicial ?>&data_final=<?=$data_final ?>&cliente=<?=$cliente ?>&nome_cliente=<?=$nome_cliente ?>&cliente2=<?=$cliente2 ?>&nome_cliente2=<?=$nome_cliente2 ?>&buscar=<?=$buscar ?>&tipo=<?=$tipo ?>&busuario=<?=$busuario ?>&bstatus=<?=$bstatus ?>';">
             </div>
           </form></td>
        <tr>
  </table>
