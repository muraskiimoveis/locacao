...
if($lista == "")
	...
	if($list == ""){
	$query1 = "select * from muraski where (finalidade='8' OR finalidade='9' OR finalidade='10' OR finalidade='11' OR finalidade='12' OR finalidade='13' OR finalidade='14' OR finalidade='15' OR finalidade='16' OR finalidade='17') and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' order by ref limit $from, 30";
	}
	else
	{
		
	$query1 = "select * from muraski where $campo like '%$chave%' 
	and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' order by ref limit $from, 30";
...
<div align="center">
  ...
  <form name="form1" id="form1" method="get" action="p_list_imoveis2.php">
    <tr>
		<td bgcolor="#<?=$cor3 ?>" colspan=5 class="style1" align="center"><b>Relação de Imóveis</b></td>
	</tr>
    <tr bgcolor="#<?php print("$cor1"); ?>">
      <td colspan=4 class="style1"><b><? if($_GET['campo']=='end'){ echo("Endereço:"); }else{ echo("Palavra Chave:"); } ?></b> <? if($_GET['campo']=='end'){ ?>
      ...
      <? } ?> 
	  <select name="campo" class="campo" onChange="form1.action='p_list_imoveis2.php?list=<? echo(""); ?>&lista=<? echo(""); ?>';form1.submit();">
...
<input type="button" onClick="window.opener.document.form1.con_ref_inquilino.value='<?php print("Ref.: $not[ref] - ".$not[condominio]." - ".$not[tipo_logradouro]." ".$not[end]." ".$not[numero].""); ?>';window.opener.document.form1.ap_inquilino.value='<?php print($not[apto]); ?>';window.opener.focus(); window.close();" class="campo3" value="Selecionar"></td>
<!--a href="#" onClick="window.opener.document.form1.con_ref_inquilino.value='<?php print("Ref.: $not[ref] - ".strip_tags($titulo).""); ?>';window.opener.focus(); window.close();" class="style1">Selecionar</a></td-->
</tr>
<?php
	}
	}
	...
	from muraski where (finalidade='8' OR finalidade='9' OR finalidade='10' OR finalidade='11' OR finalidade='12' OR finalidade='13' OR finalidade='14' OR finalidade='15' OR finalidade='16' OR finalidade='17') and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
	}else{
	$query2 = "select count(cod) as contador 
...
/*
	for ($i = 1; $i <= $pages; $i++) {
  	...
  	$url2 = $PHP_SELF . "?screen=" . $i . "&tipo1=" . $tipo1 . "&campo=" . $campo . "&chave=" . $chave . "&query_finalidade=" . $query_finalidade . "&list=" . $list;
