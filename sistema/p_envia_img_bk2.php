	...
	if ($foto != ""){
         ...
         $refe = $t_nome[0];
			...
			//Insere o usuário que esta fazendo a inserção da imagem
   			$insere = mysql_query("INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_imovel, a_ref, a_acao, a_data, a_hora) VALUES ('".$_SESSION['cod_imobiliaria']."','".$u_cod."','".$refe."','".$refe."','".$B1."','".$data."','".$hora."')") or die ("Erro 370 - ".mysql_error());
      ...
      $ref = $ref2[0];
		...
		for($j = 0; $j <= $i; $j++)
		{
         ...
         ### Cadastro da alteração
 			$insere = mysql_query("INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_imovel, a_ref, a_acao, a_data, a_hora) VALUES ('".$_SESSION['cod_imobiliaria']."','".$u_cod."','".$ref."','".$ref."','Reordenar Imagem','".date("Y-m-d")."','".date("H:i:s")."')") or die ("Erro 370 - ".mysql_error());
...
<?	if($REQUEST_METHOD!="POST") { ?>
	...
	<tr>
	  <td colspan="2" class="style1" align="left">
		...
		<? }else{ ?>
		- O tamanho maximo das fotos grandes é de 900x674pixels. Imagens maiores serão redimensionadas.<br>
		<? } ?>
		...
		<span class="style7"><b>- Somente é ACEITO imagens com as seguintes extensões: JPG ou JPEG.</b></span><br> 
	...
	<tr>
	  <td align="right" colspan="2"><input type="hidden" id="bfotos" name="bfotos" value="0">
...
<br>
				...
				<tr>
				<td colspan="2" bgcolor="#<?php print("$cor3"); ?>" class="style1">Pesquisar Imagem:</b></td>
				</tr>
				<tr bgcolor="#ffffff">
...
<? }
	$url3 = $PHP_SELF . "?screen=" . $k . "&chave=" . $chave . "&pesq=" . $pesq . "&ordem=" . $ordem . "&finalidade2=" . $finalidade2 . "&B1=Apagar Imagem" . "&foto=" . $arquivos[$j] . "&tmp_fold=" . $fold_arquivos[$j];
?>
	<td><a href="#" onClick="if (confirm('Deseja Realmente excluir o arquivo \'<?php print("$arquivos[$j]"); ?>\'?')) { window.location='<?=$url3 ?>'; }" class="style1">Apagar</a></td>
</tr>
<?php
...
?>
                  ...
                  <? } ?>
				  <tr>
                  	<td colspan="3" bgcolor="<?php print("$cor3"); ?>" class=style1 align=center>
...
<?php
	if ($from > $quant_pag) {
	$url1 = $PHP_SELF . "?screen=" . ($screen - 1) . "&chave=" . $chave . "&pesq=" . $pesq . "&ordem=" . $ordem . "&finalidade2=" . $finalidade2;
?>
...
<?php
	...
	{
		$url3 = $PHP_SELF . "?screen=" . ($screen + 1) . "&chave=" . $chave . "&pesq=" . $pesq . "&ordem=" . $ordem . "&finalidade=" . $finalidade2;
?>
...
<?php
	...
	{
		$url3 = $PHP_SELF . "?screen=" . ($screen + 1) . "&chave=" . $chave . "&pesq=" . $pesq . "&ordem=" . $ordem . "&finalidade2=" . $finalidade2;
?>
                  <a href="<?php print("$url3"); ?>" class="style1">>> Próxima Página >></a>
<?php
	}
