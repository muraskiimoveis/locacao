...
if($B1 == "Atualiza Imagem") {
	...
	if ($foto != ""){
		...
		$sqlinsere .=  " ('".$_SESSION['cod_imobiliaria']."','".$u_cod."','".$refe."', '".$refe."','".$tipo_atualiza."','".$data."','".$hora."')";
   		$insere = mysql_query($sqlinsere) or die ("Erro 370 - ".mysql_error());
...
if($_POST['acao_site']=='1')
{
		...
		for($p = 0; $p <= $y; $p++)
		{
			...
			if($botao=='1')
			{
				...
				$refer = $tref[0];
				$sqlinsere = "INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_imovel, a_ref, a_acao, a_data, a_hora) VALUES ";
				$sqlinsere .= " ('".$_SESSION['cod_imobiliaria']."','".$u_cod."','".$refer."','".$refer."','Atualiza Imagem','".date("Y-m-d")."','".date("H:i:s")."')";
				$insere = mysql_query($sqlinsere) or die ("Erro 370 - ".mysql_error());
...
<?	}else{?>
	...
	<input type="checkbox" name="enviarsite_foto_<?=$j ?>" id="enviarsite_foto_<?=$j ?>" value="1"><a href="#" onClick="if (confirm('Deseja Realmente Atualizar a Foto \'<?php print("$arquivos[$j] no Site" ); ?>\'?')) { window.location='<?=$url_site ?>'; }" class="style1"><? echo $tipo_envio_site; ?></a></td>
<?	}?>
	<td width="20%" colspan="2" align="center">
	 <input type="hidden" name="foto_<?=$j ?>" id="foto_<?=$j ?>" value="<?=$arquivos[$j] ?>">
...
?>
			...
			<tr>
				...
				<input type="button" value="Confirmar o Envio/Retirada do Site das fotos Selecionadas" name="bt_enviarsite" id="bt_enviarsite" class="campo3" Onclick="ValidaEnvioSite('<?=$chave ?>', '<?=$finalidade2 ?>', '<?=$pesq ?>', '<?=$ordem ?>', '<?=$screen ?>');">
			</td>
