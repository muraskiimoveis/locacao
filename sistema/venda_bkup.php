...
<?php	
	...
	$valores = explode(".00", $not1[valor]);
	$valor = $valores[0];
	
	//$total = ($total_dias * $not1[valor]);
	$totais = explode(".00", $not1[valor]);
	$total = $totais[0];
	...
	$valores2 = explode(".00", $not1[valor]);
	$valor2 = $valores2[0];
	$com = $not1[comissao];
...
?>
    ...
    <tr bgcolor="#<?php print("$cor6"); ?>">
      <td colspan=2><table>
      	<tr>
      		<td class=style1>Parcela <?php print("$j"); ?>:  R$ <input type="text" name="parc[<?php print("$j"); ?>]" value="<?php print("$parc[$j]"); ?>" size="10" class="campo"> <input type="checkbox" name="marca[<?php print("$j"); ?>]" value="ok" <?php if($marca[$j] == "ok"){ echo "checked"; } ?>> <i>Fixa valor</i> Data: <input type="text" name="dia_parc[<?php print("$j"); ?>]" size="2" class="campo" value="<?php print("$dia_parc[$j]"); ?>" maxlenght="2" onKeyUp="return autoTab(this, 2, event);">/<input type="text" name="mes_parc[<?php print("$j"); ?>]" size="2" class="campo" maxlenght="2" onKeyUp="return autoTab(this, 2, event);" value="<?php print("$mes_parc[$j]"); ?>">/<input type="text" name="ano_parc[<?php print("$j"); ?>]" size="4" class="campo" maxlenght="4" onKeyUp="return autoTab(this, 4, event);" value="<?php print("$ano_parc[$j]"); ?>"> 
...
?>
      			...
      			</select> <input type="text" name="dados_banco[<?php print("$j"); ?>]" value="<?php if($dados_banco[$j] != ""){ print("$dados_banco[$j]"); }else{ echo "Dados do cheque"; } ?>" size="30" class="campo" onFocus="if(this.value=='Dados do cheque')this.value='';"><br><b>OBS.: Prrenchimento Obrigatório.</b></td>
    </tr></table></td>
  </tr>
<?php
	}
...
?>
    <tr bgcolor="#<?php print("$cor1"); ?>">
      ...
      <input type="submit" value="Atualizar Valores" class=campo name="B1"> <b>OBS.: Deve ser clicado sempre que algum dado for alterado.</b></td>
    </tr>
    </form>
<script language="javascript">
function valida_usu()
{
  if (formulario.usuario.value == "")
  {
    ...
    <form method="get" action="p_venda.php" name="formulario" id="formulario" onSubmit="return valida_usu();">
...
?>
> Depósito no valor de R$ <?php print("$parc[$j]"); ?> a ser efetuado até dia <?php print("$dia_parc[$j]"); ?>/<?php print("$mes_parc[$j]"); ?>/<?php print("$ano_parc[$j]"); ?> Muraski Imóveis Ltda, Banco do Brasil Ag.: 2.100-8 C/C.: 44210-0.
<?php
...
?></textarea></td>
    ...
    <tr bgcolor="#<?php print("$cor6"); ?>">
      ...
      <td width="80%" class=style1><input type="text" name="co_cliente" size="4" class="campo2" readonly> <input type="text" name="nome_cliente" size="40" class="campo" readonly>
      	<input type="button" value="Selecionar" id="selecionar2" name="selecionar2" class="campo" onClick="window.open('p_list_comprador.php', 'janela', 'height=500,width=730,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no');">
        <br>
        Se o comprador não está cadastrado <a target="_blank" href="p_insert_clientes.php" class="style1">clique aqui</a>.</td>
    </tr>
	</tr>
	<tr bgcolor="#<?php print("$cor6"); ?>">
      <td width="20%" class=style1><b>Usuário:</b></td>
      <td width="80%" class="style1"><input type="text" name="usuario" size="40" class="campo" readonly value="<?php print("$u_nome"); ?>"></td>
    </tr>
    <tr bgcolor="#<?php print("$cor1"); ?>">
      <td width="100%" colspan=2>
      <input type="hidden" value="<?php print("$not1[cod]"); ?>" name="cod">
      <input type="submit" value="Inserir Compra" class=campo name="B1"></td>
    </tr>
  </table>
  </form>
<br>
