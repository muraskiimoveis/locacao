...
*/
	...
	if($alterar == 1){
	if($bot == "Apagar"){
    if (verificaFuncao("DEP_USER")) { // verifica se pode acessar as areas 
...
<?php
	$query7 = "SELECT * FROM despesas WHERE de_imovel='$cod' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' and de_status='à receber' 
	order by de_data desc";
	//echo $query7;
	$result7 = mysql_query($query7);

	while($not7 = mysql_fetch_array($result7))
	{
		$de_valor = $not7[de_valor];
		$de_valor = number_format($de_valor, 2, ',', '.');
		
		$despesas = $despesas + $not7[de_valor];
		$desp_total = number_format($despesas, 2, ',', '.');
		
			$ano7 = substr ($not7[de_data], 0, 4);
		  $mes7 = substr($not7[de_data], 5, 2 );
		  $dia7 = substr ($not7[de_data], 8, 2 );
?>
<tr><td bgcolor="#EDEEEE" colspan=4><font color="#ff0000" size="2" face="Arial">
<?php print("$dia7/$mes7/$ano7"); ?> - <?php print("$not7[de_desc]"); ?> - <?php print("$not7[de_valor]"); ?></td>
</tr>
<?php
	}//termina despesas à receber
?>
<tr><td bgcolor="#EDEEEE" colspan=4><font color="#ff0000" size="2" face="Arial">
<b>Total de despesas à receber: <?php print("$desp_total"); ?></td>
</tr>
<?php
	$query6 = "SELECT * FROM depositos WHERE d_imovel='$cod' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' order by d_imovel desc, d_data desc 
	limit 1";
	//echo $query6;
...
<?php
	...
	{
?>
Apenas o administrador pode atualizar ou apagar Depósitos!
<?php
	}
?></td>
