...
function valida()
{
  ...
  if (document.form1.cont.selectedIndex == 0 && document.form1.contador.selectedIndex == 0)
  {
        ...
        {
            if (inputs[i].value == ''){
                alert('Por favor, preencha todos os proprietários selecionados');
                inputs[i].focus();
                return false;
            }
        }  
...
<p align="center" class=style1><b>Editar ou Apagar Imóvel</b></p></td></tr>
    ...
    <tr bgcolor="#EDEEEE">
      <td width="35%" class=style1><b>Proprietário: N° de campos: <select name="cont" id="cont" class="campo" onChange="form1.action='p_edit_imoveis.php?cod=<?php echo($cod); ?>&edit=editar';form1.submit();">
...
<?
			...
			if($_POST['cont']){
		     ...
		     for($i3 = 1; $i3 <= $contador; $i3++){
			   ...
			   echo(" 			   		
           			<input type=\"text\" name=\"nome_cliente_$i3\" id=\"nome_cliente_$i3\" size=\"50\" class=\"campo\" value='$nome_cliente3' readonly>
           			<input type=\"button\" id=\"selecionar2\" name=\"selecionar2\" value=\"Selecionar\" class=\"campo3\" onClick=\"window.open('p_list_proprietario2.php?id=".$i3."', 'janela', 'height=500,width=730,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no');\"><a href=\"p_edit_imoveis.php?cod=".$cod."&edit=editar&contador=".$contador."&cont=".$cont."\" class=\"style1\"> [ - ] Remover</a><br><b>Dados do Prop.: </b><a href=\"p_clientes.php?lista=1&c_cod=".$cliente2[$i3-1]."\" class=\"style1\">Clique aqui para visualizar os dados do cliente</a><br>
			   ");
			 }
			 echo("");  
		   ...
		   if($cont){
		     for($i = 1; $i <= $cont; $i++){
			   echo("
			   		<input type=\"text\" name=\"cliente4_$i\" id=\"cliente4\" size=\"5\" class=\"campo2\" value=\"".$cliente4."\" readonly>
           			<input type=\"text\" name=\"nome_cliente4_$i\" id=\"nome_cliente4\" size=\"50\" class=\"campo\" value='$nome_cliente4' readonly>
           			<input type=\"button\" id=\"selecionar2\" name=\"selecionar2\" value=\"Selecionar\" class=\"campo3\" onClick=\"window.open('p_list_proprietario3.php?id=".$i."', 'janela', 'height=500,width=730,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no');\"><br>
			   ");
			 }  
		   }
?>         
            <input type="hidden" name="contador" id="contador" size="5" class="campo2" value="<?=$contador ?>">
		   </td>
    </tr>
    <tr bgcolor="#EDEEEE">
