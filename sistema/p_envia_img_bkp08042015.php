...
if($B1 == "Apagar Imagem") {
	if ($foto != ""){
		...
		if (file_exists($pastap . $foto_peq)) {
            ...
            , a_acao, a_data, a_hora) VALUES ('".$_SESSION['cod_imobiliaria']."','".$u_cod."','".$nref[0]."'
            , '".$nref[0]."','".$B1."','".$data."','".$hora."')";
...
if($_POST['acao_apagar']=='1')
{
		...
		for($p = 0; $p <= $y; $p++)
		{
            ...
            $tref = explode("_",$foto);
            $refer = $tref[0];
    			$insere = mysql_query("INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_imovel, a_ref, a_acao, a_data, a_hora) VALUES ('".$_SESSION['cod_imobiliaria']."','".$u_cod."','".$refer."','".$refer."','Apagar Imagem','".date("Y-m-d")."','".date("H:i:s")."')") or die ("Erro 370 - ".mysql_error());
...
<?
		...
		$SQL = "SELECT ref FROM muraski WHERE ref='".$ref."' and $finalidadev AND cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
...
<form method="post" action="" name="form3">
                 <table width="75%" align="center" cellpadding="1" cellspacing="1">
				 	<tr bgcolor="#<?php print("$cor3"); ?>" height="15">
  						...
  						<td width="60%" class="style1" align="center"><b>Descri&ccedil;&atilde;o</b></td>
...
<? }else{ ?>
	<td width="60%" class="style1" align="left" style="text-indent: 5px;"><?php print($arquivos[$j]); ?></td>
<? }
...
?>
                  <? if($chave != "" && $finalidade2 != ""){ ?>
				  <tr>
                  	<td colspan="4" bgcolor="<?php print("$cor1"); ?>" class=style1 align=right>
                  	<? if($_POST['editar']=='1'){  ?>
                  ...
                  <tr>
					...
					<input type="button" value="Apagar fotos selecionadas" name="bt_apagar" id="bt_apagar" class="campo3" Onclick="Validacao('<?=$chave ?>', '<?=$finalidade2 ?>', '<?=$pesq ?>', '<?=$ordem ?>', '<?=$screen ?>');"><br><br></td>
                  </tr>
				  <tr>
