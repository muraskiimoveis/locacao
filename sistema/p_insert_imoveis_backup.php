...
function valida()
{
  ...
  if (document.form1.cont.selectedIndex == 0)
  {
		...
		if (document.form1.comissao_prop.value == "")
  		{
    		alert("Por favor, preencha o campo Comissão Proprietário");
    		document.form1.comisao_prop.focus();
    		return false;
  		}  
  }
...
if($B1 == "Inserir Imóvel")
	...
	, ftxt_16, ftxt_17, ftxt_18, ftxt_19, ftxt_20, cliente, percentual_prop, matricula, cidade_mat
	...
	, '$ftxt_18', '$ftxt_19', '$ftxt_20', '".$cliente."', '".$percentual."', '$matricula', '$cidade_mat'
	...
	, '$valor_oferta', '$relacao_bens', '".$observacoes."', '".$disponibilizar."', '".$disp_rede."', '".$destaque."', '".$comissao_parceria."', '$cont')";
	...
	, angariador, zelador, tipo_anga, indicador, comissao_indicador, comissao_vendedor, chaves, controle_chave, tipo_div, valor_oferta, relacao_bens, observacoes, disponibilizar, disp_rede, destaque, comissao_parceria, contador) 
	values('".$_SESSION['cod_imobiliaria']."', '$ref', '$tipo1', '$metragem', '".$area_terreno."', '".$matricula_luz."'
	, '".$situacao_luz."', '".$matricula_agua."', '".$situacao_agua."', '$n_quartos'
	, '$valor', '$especificacao', '$suites', '".$caracteristica1."', '$piscina'
	, '$titulo', '$desc', '$uf', '$local', '$permuta', '$finalidade', '$permuta_txt'
...
function Dados(valor) {
  ...
  <form method="post" name="form1" id="form1" onSubmit="return valida();" action="<?php print("$PHP_SELF"); ?>">
	...
	<tr bgcolor="#EDEEEE">
      ...
      <td width="67%" class=style1> <input type="text" name="ref" id="ref" size="10" maxlength="10" class="campo" value="<?=$ref; ?>" onKeyUp="return autoTab(this, 10, event);"> <a href="p_ref.php" target="_blank" class=style1>Ver referências: usadas e disponíveis</a></td>
    </tr>
	<tr bgcolor="#EDEEEE">
      <td width="33%" class=style1><b>Proprietário: N° de campos: <select name="cont" id="cont" class="campo" onChange="form1.action='p_insert_imoveis.php';form1.submit();">
           ...
           if($_POST['cont']){
		     for($i = 1; $i <= $_POST['cont']; $i++){
			   ...
			   echo("
           			...
           			<input type=\"button\" id=\"selecionar2\" name=\"selecionar2\" value=\"Selecionar\" class=\"campo3\" onClick=\"window.open('p_list_proprietario2.php?id=".$i."', 'janela', 'height=500,width=730,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no');\">
           			<b>Percentual:</b>
...
<? } ?>
    ...
    <tr bgcolor="#EDEEEE">
      ...
      <td class="style1"><select name="tipo_logradouro" id="tipo_logradouro" class="campo">
        ...
        <input type="text" name="ende" id="ende" size="50" class="campo" value="<?=$ende; ?>">
        <br>
        <span class="style7">Para que o mapa funcione corretamente sem precisar preencher as coordenadas do Google Maps no campo "Coordenadas" &eacute; preciso o tipo de logradouro + endere&ccedil;o completo  e o campo n&uacute;mero preenchidos  (Exemplo: Avenida 29 de Abril 601)</span></td>
    </tr>
    <tr bgcolor="#EDEEEE">
    ...
    <tr bgcolor="#EDEEEE">
      ...
      <td class=style1><input name="cep" type="text" class="campo" id="cep" value="<?php print("$cep"); ?>" size="8" maxlength="8"> 
      Exemplo: 80000000 </td>
      </tr>
    <tr bgcolor="#EDEEEE">
...
?>    
    <tr bgcolor="#EDEEEE">
      ...
      <td class="style1"><input name="disponibilizar" type="radio" id="disponibilizar1" value="0" <? if($disponibilizar=='0'){ print "CHECKED"; } ?> checked>
        N&atilde;o</td>
    </tr>
<?
}else{
