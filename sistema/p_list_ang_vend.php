...
if($lista == "")
	...
	if($list == ""){
		$query1 = "select * from usuarios WHERE cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' order by u_nome limit $from, 30";
	}else{
		$query1 = "select * from usuarios where cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND $campo like '%$c_nome%' order by u_nome 
		limit $from, 30";
	}
	//echo $query1;
...
<table border="0" cellpadding="1" cellspacing="1" width="95%">
	<tr height="50">
		<td colspan=5 class="style1"><p align="center"><b>Relação de Angariadores e Vendedores</b></p></td>
	</tr>
	<form method="get" action="p_list_ang_vend.php">
      ...
      <option value="u_nome">Nome</option>
	...
	<tr class="fundoTabelaTitulo">
		<td width="55%" class="style1"><b>Nome</b></td>
...
<td class="style1">
<p align="center">
<input type="button" onClick="window.opener.document.form1.cliente2.value='<?php print("$not[u_cod]"); ?>';window.opener.document.form1.nome_cliente2.value='<?php print("$not[u_nome]"); ?>';window.opener.focus(); window.close();" class="campo3" value="Selecionar"></td>
<!--a href="#" onClick="window.opener.document.form1.cliente.value='<?php print("$not[u_cod]"); ?>';window.opener.document.form1.nome_cliente.value='<?php print("$not[u_nome]"); ?>';window.opener.focus(); window.close();" class="style1">Selecionar</a></td-->
<?php
	}
	
	if($list == ""){
	$query2 = "select count(u_cod) as contador from usuarios WHERE cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
	}else{
		$query2 = "select count(u_cod) as contador from usuarios where cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' AND $campo like '%$c_nome%'";	
	}
...
?>
  <tr>
  	<td colspan="5" class="fundoTabelaTitulo style1"><p align="center"><b>Foram encontrados <?php print("$not2[contador]"); ?> vendedores/angariadores</b></td>
  </tr>
  <tr>
