...
function confirmaAtivar(cod)
{
       if(confirm("Tem certeza que deseja ativar esse imóvel para venda?"))
          document.location.href='p_imoveis.php?cod=' + cod;
}
...
<form  name="form1" id="form1" method="post" action="p_imoveis.php">
...
var msgErro = '';
		...
		else
		{
  		...
  		if(document.form1.cidade_mat.selectedIndex==0)
  		{
			...
			document.form1.cidade_mat.style.backgroundColor = '<?=$cor_erro ?>';
			return false;
		}
		else
		{
		  document.form1.cidade_mat.style.backgroundColor = '<?=$cor1 ?>';
		}
        ...
        else
        {
  		...
  		if(document.form1.n_quartos.value.length==0)
  		{
		...
		else
		{
  		...
  		if(document.form1.controle_chave.value.length==0)
  		{
		...
		else
		{
  		...
  		if(document.form1.chaves.value.length==0)
  		{
...
<p align="center" class=style1><b>Editar ou Apagar Imóvel</b></p></td></tr>
	...
	<tr bgcolor="#EDEEEE">
      ...
      <td width="65%" class=style1> <input type="text" class="campo" name="ref" id="ref" size="10" maxlength="10" value="<?php print("$not2[ref]"); ?>" onKeyUp="return autoTab(this, 10, event);"> <a href="javascript:;" onClick="MM_openBrWindow('p_ref.php?menu=N','','scrollbars=yes,resizable=no,width=800,height=600')" class="style1"><b>Ver referências: usadas e disponíveis</b></a></td>
   </tr>
	<!--tr bgcolor="#EDEEEE">
			...
			if($_POST['cont']){
		     ...
		     for($i3 = 1; $i3 <= $contador; $i3++){
			   ...
			   echo("
           			...
           			<input type=\"button\" id=\"selecionar2\" name=\"selecionar2\" value=\"Selecionar\" class=\"campo3\" onClick=\"window.open('p_list_proprietario2.php?id=".$i3."', 'janela', 'height=500,width=730,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no');\">
           			<b>Percentual:</b>
					<input type=\"text\" name=\"percentual_$i3\" id=\"percentual_$i3\" size=\"2\" class=\"campo\" value=\"".$percentual2[$i3-1]."\">%
				");
			   if($contador=='1'){
			       echo("<br>");
			   } 
			   ...
			   echo "<b>Dados do Prop.: </b><a href=\"p_clientes.php?lista=1&c_cod=".$cliente2[$i3-1]."\" class=\"style1\">Clique aqui para visualizar os dados do cliente</a><br>";
			 }
		   ...
		   if($cont){
		     for($i = 1; $i <= $cont; $i++){
			   ...
			   echo("
           			...
           			<input type=\"button\" id=\"selecionar2\" name=\"selecionar2\" value=\"Selecionar\" class=\"campo3\" onClick=\"window.open('p_list_proprietario3.php?id=".$i."', 'janela', 'height=500,width=730,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no');\">
           			<b>Percentual:</b>
...
?>         
	...
	<tr bgcolor="#EDEEEE">
      <td width="35%" class=style1><b>Idade do Imóvel:</b></td>
      <td width="65%" class=style1>
         <input type="text" class="campo" size="3" maxlength="3" name="idade_imovel" id="idade_imovel" value="<?php if($_POST['idade_imovel']){ print($_POST['idade_imovel']); }else{ print($not2[idade_imovel]); } ?>"></td>
   </tr>
	<tr bgcolor="#EDEEEE">
   ...
   <tr bgcolor="#EDEEEE">
      <td width="35%" class=style1><b>Apto.:</b></td>
      <td width="65%" class=style1>
         <input type="text" class="campo" size="5" name="apto" id="apto" value="<? if($_POST['apto']){ print($_POST['apto']); }else{ print($not2[apto]); } ?>"></td>
   </tr>
    <!--tr bgcolor="#EDEEEE">
...
?>
   <tr bgcolor="#<?php print("$cor1"); ?>"><td colspan=2 class="style1">
    <p align="left"><b><a href="p_rel_loc_mes.php?cod=<?php print("$not2[cod]"); ?>" class=style1>Visualizar Relatório de Locações</a></b></p></td>
  </tr>
...
<? } ?>
  <tr bgcolor="#<?php print("$cor1"); ?>"><td colspan=2 class="style1">
    <p align="left"><b><a href="#" onClick="NewWindow('','uprelatorio','900','500','yes');form1.target='uprelatorio';form1.action='p_extrato_locacao.php?cod_imovel=<?php print("$not2[cod]"); ?>';form1.submit();" class=style1>Extrato de Locações</a></b></p></td>
  </tr>
<?php
	}
	else
...
{
window.open("fazer_sinal.php?cod=<?php print($not2[cod]); ?>&codi=<?php print($_SESSION['cod_imobiliaria']); ?>","1",'toolbar=yes,location=0,directories=0,status=0,menubar=0,scrollbars=yes,resizable=0,width=900,height=400');
}
</script>
	<tr bgcolor="#<?php print("$cor1"); ?>"><td colspan=2 class="style1">
...
<? } ?>
	<!--tr bgcolor="#<?php print("$cor1"); ?>">
      <td colspan=2 class="style1"><b><a href="#" onClick="NewWindow('','uprelatorio','800','600','yes');form1.target='uprelatorio';form1.action='cadastro_servicos.php?cod=<?php echo("$not2[cod]"); ?>';form1.submit();" class=style1>Serviços de Cobrança</a></b></td>
    </tr-->
    <tr bgcolor="#EDEEEE">
...
?>
    <tr bgcolor="#EDEEEE">
     ...
     <tr bgcolor="#EDEEEE">
      ...
      <td class="style1"><select name="tipo_logradouro" id="tipo_logradouro" class="campo">
        <option value="">Selecione</option>
      ...
      <input type="text" name="ende" id="ende" size="50" class="campo" value="<?php print("$not2[end]"); ?>"> <b>N&uacute;mero:</b> <input type="text" name="numero_end" id="numero_end" size="5" class="campo" value="<?php print("$not2[numero]"); ?>"><br>
      <b>CEP:</b> <input name="cep" type="text" class="campo" id="cep" value="<?php print("$not2[cep]"); ?>" size="8" maxlength="8">
Exemplo: 80000000 <br>
      <span class="style7">Para que o mapa funcione corretamente sem precisar preencher as coordenadas do Google Maps no campo "Coordenadas" é preciso o tipo de logradouro + endereço completo, o campo n&uacute;mero e o CEP preenchidos. (Exemplo: Avenida 29 de Abril 601 CEP: 80000000)</span></td>
    </tr>
        <tr bgcolor="#EDEEEE">
...
Exemplo: 80000000 <br>
     ...
     <!--tr bgcolor="#EDEEEE">
      ...
      <td class="style1"><input name="end_aproximado" type="checkbox" id="end_aproximado" value="1" <? if($not2[end_aproximado]=='1'){ print "CHECKED"; } ?>>
        Sim<br><span class="style7">Obs: Se este campo estiver marcado o ícone no mapa do Google ficará um pouco maior.</span></td>
    </tr>     
    <tr bgcolor="#EDEEEE">
...
?>        
    ...
    <? }else{ ?>
      ...
      <? } ?> 
<?
	 $result30 = mysql_query("SELECT im_disponibilizar FROM rebri_imobiliarias WHERE im_cod='".$_SESSION['cod_imobiliaria']."'");
...
?>
	...
	<tr bgcolor="#EDEEEE">
	  ...
	  <select name="comissao_parceria" id="comissao_parceria" class="campo" onChange="form1.action='p_edit_imoveis.php?cod=<?php echo($cod); ?>&edit=editar&m=1';form1.bcom.value='1';form1.submit();">
