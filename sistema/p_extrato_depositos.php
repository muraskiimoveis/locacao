...
<?php
	...
	if($imprimir_boleto=="Re-Imprimir"){
		...
		//die();
		$exec_busca_boleto = mysql_query($busca_boleto) or die("Não foi possível pesquisar Contas.");
	...
	if($novo_boleto=="Novo Boleto"){
		...
		while($not_bol = mysql_fetch_array($exec_busca_boleto))
		{
			...
			//
			$query_contas= "update contas set co_data='".$data_venc."', co_boleto = '".str_pad($nrdoc,11,"0",STR_PAD_LEFT)."', co_status = 'pendente' where co_boleto='".str_pad($nrdoc_ant,11,"0",STR_PAD_LEFT)."' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
			$resul_contas = mysql_query($query_contas) or die("Não foi possível atualizar suas informações. $query_contas");
			//
			$query_bol= "update boleto set dt_baixa='".$dt_baixa."', bol_baixado = 'S' where nrdoc='".str_pad($nrdoc_ant,11,"0",STR_PAD_LEFT)."'";
			$resul_bol = mysql_query($query_bol) or die("Não foi possível atualizar suas informações. $query_bol");
			//
			...
			$insere_novo_boleto .= "values ('".$sacado."','".str_pad($nrdoc,11,"0",STR_PAD_LEFT)."','".date("Y-m-d", mktime(0, 0, 0, $mes, $dia, $ano))."',".number_format($valor,2,".","").",'".$d_prazo."','".$taxa."',";
			...
			$insere_novo_boleto .= "'".$razao."','".$razao_cnpj."','".$razao_end."','".$razao_local."','".$url_impbol[0]."','".$bol_status."')";
            			$rs_insere_boleto = mysql_query($insere_novo_boleto) or die("Não foi possível atualizar suas informações. $insere_novo_boleto $url_impbol[0]");
			//
			// Fim do Boleto
		...
		$exec_busca_remessa = mysql_query($busca_remessa) or die("Não foi possível pesquisar Contas.");
		...
		while($not_rem = mysql_fetch_array($exec_busca_remessa))
		{
			...
			//echo "Url_Remessa ==> ".$url_imprem[0]."<BR>";
            			...
            			// Fim Guarda
			//
			$query_rem= "update remessa set dt_baixa='".$dt_baixa."', rem_baixada = 'S' where nrdoc='".str_pad($nrdoc_ant,11,"0",STR_PAD_LEFT)."'";
			$resul_rem = mysql_query($query_rem) or die("Não foi possível atualizar suas informações. $query_rem");
			//
			...
			$insere_remessa .= "values ('".$nome."','".str_pad($nrdocr,11,"0",STR_PAD_LEFT)."','".date("Y-m-d", mktime(0, 0, 0, $mes, $dia, $ano))."',".number_format($valor_boleto,2,".","").",";
...
<?php
		...
		while($not6 = mysql_fetch_array($result18))
		{
			...
			$query22 = "select SUM(co_valor) as saldo from contas where co_imovel='$not6[cod]' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' and co_cat='Receber' AND co_tipo='Despesas Imóvel' and co_status='pendente'";
...
<tr class="fundoTabela">
	<td colspan=7><table width=100%>
		<tr>
			<td class=style1><b><?php print("$data_ent"); ?></b> à <b><?php print("$data_sai"); ?></b></td>
			<td class=style1><b>Locatário:</b> <a href="p_clientes.php?lista=1&c_cod=<?php echo($c_cod2); ?>" class="style1" target="_blank"><?php print("$locatario"); ?></a></td>
			<td class=style1><b>Valor Total:</b> R$ <?php print("$total_tela"); ?></td>
		</tr>
	</table></td>
</tr>
<tr height="50">
	<td colspan=7 class=style1 align="center"><b>Valores à receber do Locatário:</b></td>
</tr>
<?php
	if($total_credp7 != 0){
...
<tr class="fundoTabela">
	<td colspan=7><table width=100%>
		<tr>
			<td class=style7 align=center><b>Existe saldo anterior pendente: R$ <?php print("$total_credp_tela7"); ?></b> - <a href="#" onclick="NewWindow('p_extrato_depositos.php?locacao=<?php print("$locacao_ant"); ?>&tipo_pesq=Locação', 'janela2', 750, 500, 'yes')" class="style1">Visualizar</a></td>
		</tr>
	</table></td>
</tr>
<?php
...
<tr class="fundoTabela">
	<td colspan=7><table width=100%>
		<tr>
			<td class=style7 align=center><b>Existe saldo anterior pendente: R$ <?php print("$total_cred_desp_tela"); ?></b> - <a href="#" onclick="NewWindow('p_extrato_depositos.php?locacao=<?php print("$locacao_ant"); ?>&tipo_pesq=Locação', 'janela3', 750, 500, 'yes');" class="style1">Visualizar</a></td>
		</tr>
	</table></td>
</tr>
<?php
...
<tr class="fundoTabela">
	<td colspan=7><table width=100%>
		<tr>
			<td class=style7 align=center><b>Existe saldo anterior pendente: R$ <?php print("$total_debp_tela7"); ?></b> - <a href="#" onclick="NewWindow('p_extrato_depositos.php?locacao=<?php print("$locacao_ant"); ?>&tipo_pesq=Locação', 'janela4', 750, 500, 'yes')" class="style1">Visualizar</a></td>
		</tr>
	</table></td>
</tr>
<?php
