...
<?php
    ...
    $valor1 = str_replace(".", "", $co_valor);
    $valor2 = str_replace(",", ".", $valor1);

    $valor1p = str_replace(".", "", $co_valorp);
    $valor2p = str_replace(",", ".", $valor1p);
	...
	for($j = 1; $j <= $contador; $j++){
		...
		values('".$_SESSION['cod_imobiliaria']."','$clientes', 'Receber', '$cod', '$co_desc', 'Despesas', '".formataDataParaBd($data_despesa)."', 'ok', '$valor'
	...
	, co_locacao, co_usuario, co_forma, co_data_status, co_usuario_status) 
	values('".$_SESSION['cod_imobiliaria']."', '$prestador', 'Pagar', '$cod', '$co_desc', 'Despesas', '".formataDataParaBd($data_despesa)."', 'pendente', '$valor2p'
	, '$l_cod', '$u_cod', 'Dinheiro', '".formataDataParaBd($data_despesa)."', '$u_cod')";
...
?>
<form method="get" name="form2" action="<?php print("$PHP_SELF"); ?>">
