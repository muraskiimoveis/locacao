...
function calcula_minutos($hora1,$hora2){
...
function ver_dia_logado(){
    $qry_data = "select u_cod from logusuarios
	where u_cod='".$_SESSION["u_cod"]."' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."' and DATE_FORMAT(l_datalog, '%d-%m-%Y')='".date('d-m-Y')."'";
    $res_data = mysql_query($qry_data);
	$nrows_data = mysql_num_rows($res_data);
	$not_data = mysql_fetch_array($res_data);
    if($nrows_data == 0){
      return true;
    }else{
      return false;
...
function verificaAcesso() { // função de verificação principal =============
   ...
   */
   ...
   //VALIDA ACESSO EXTERNO
   //echo substr($_SERVER['REMOTE_ADDR'],0,11)." -- ".Trim($_SESSION['u_tipo']);
   //die("ERRO");
   if ((Trim($_SESSION['u_tipo']) == "") and
      (substr($_SERVER['REMOTE_ADDR'],0,11)!='192.168.26.') and
       ($_SERVER['REMOTE_ADDR']!='172.20.0.1') and
       ($_SERVER['REMOTE_ADDR']!='200.195.158.122') and
       ($_SERVER['REMOTE_ADDR']!='189.31.230.161'))
       {
   	echo ('<script language="javascript">alert("E-mail e/ou Senha do Usuário com status SEM ACESSO EXTERNO");document.location.href="index.php";</script>');
   }

   if (ver_dia_logado() and (Trim($_SESSION['u_cod'])!='')){
      $qry_ins_log = "INSERT logusuarios (u_cod, u_nome, cod_imobiliaria, l_datalog) VALUES ('".$_SESSION["u_cod"]."','".$_SESSION["u_nome"]."','".$_SESSION['cod_imobiliaria']."',current_timestamp)";
      $res_log = mysql_query($qry_ins_log) or die("Não foi possível atualizar suas informações. $qry_ins_log");
      $hora_banco = date('H:i',ver_hora_server());
      if (date('w',ver_hora_server()) == '0'){
        $hora_padrao = date('H:i',strtotime("09:00 +".$_SESSION['im_tolera']." minutes"));
      }else{$hora_padrao = date('H:i',strtotime("08:30 +".$_SESSION['im_tolera']." minutes"));}
      $tempo = calcula_minutos($hora_padrao,$hora_banco);
      //echo "Mostra ==> ".$tempo."<BR>";
      if ($tempo > 0){
         //echo "AKI ==> ";
         $qry_consulta = "select * from atendimento where a_corretor='".$_SESSION["u_cod"]."' and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
         $res_consulta = mysql_query($qry_consulta) or die("Não foi possível obter informações. $qry_consulta");
         $nrows_consulta = mysql_num_rows($res_consulta);
         $not_consulta = mysql_fetch_array($res_consulta);
         if($nrows_consulta > 0){
           $qry_atende = "UPDATE atendimento SET a_data_venda=current_timestamp, a_data_email=current_timestamp, a_data_telefone=current_timestamp WHERE cod_imobiliaria='".$_SESSION["cod_imobiliaria"]."' and a_corretor='".$_SESSION["u_cod"]."'";
           //$qry_atende = "UPDATE atendimento SET a_data_venda=current_timestamp WHERE cod_imobiliaria='".$_SESSION["cod_imobiliaria"]."' and a_corretor='".$_SESSION["u_cod"]."'";
         }else{
           //$qry_atende = "INSERT atendimento (a_corretor, cod_imobiliaria, a_data_venda, a_data_locacao, a_data_telefone, a_vendas, a_locacao, a_telefone) VALUES ('".$_SESSION["u_cod"]."','".$_SESSION['cod_imobiliaria']."',current_timestamp,current_timestamp,current_timestamp,'1','1','1')";
         }
         $res_atende = mysql_query($qry_atende) or die("Não foi possível obter informações. $qry_atende");
         $nrows_atende = mysql_num_rows($res_atende);
         $not_atende = mysql_fetch_array($res_atende);
         //echo ('<script language="javascript">alert("'.$tempo.'=Session=> '.$_COOKIE['rebri'].'=UserCookie=> '.' Minutos Atrasados!!");</script>');
      }
