...
if($lista == "")
	...
	$query1 = "select * from muraski where $campo like '%$chave%' $query_finalidade 
...
<div align="center">
  ...
  <form method="get" action="p_list_imoveis2.php">
...
<?php
	...
	$buscap = mysql_query("SELECT c.c_nome FROM clientes c INNER JOIN muraski m ON (c.c_cod=m.cliente) WHERE m.cod='".$not['cod']."' AND m.cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'");
...
<?php print("$not[titulo]"); ?></td><td class=style1>
<?php print($fin); ?></td><td class=style1>
<a href="#" onClick="window.opener.document.form1.con_ref_inquilino.value='<?php print("Ref.: $not[ref] - $titulo"); ?>';window.opener.document.form1.prop_inquilino.value='<?php print("$prop"); ?>';window.opener.focus(); window.close();" class="style1">Selecionar</a></td>
</tr>
<?php
	}
	}
	...
	from muraski where tipo='$tipo1' $query_finalidade and cod_imobiliaria='".$_SESSION['cod_imobiliaria']."'";
	}else{
	$query2 = "select count(cod) as contador 
