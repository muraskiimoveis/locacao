		...
		//if (verificaFuncao("USER_AREAS")) { // verifica se pode acessar as areas
			if (sizeof($areas) > 0) {
				...
				foreach($_POST["areas"] as $area_id) {
					...
					$a_result = mysql_query($a_query,$con) or die("Não foi possível inserir suas informações.");
				}
			}
		//} // fim valida acesso
		
				//controle
        $inserir_controle = "INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_acao, a_data, a_hora) VALUES ('".$cod_imob."','".$u_cod."','".$B1."',current_date,current_time)";
   		mysql_query($inserir_controle,$con) or die ("Erro 72 - ".mysql_error());
?>
...
if($B1 == "Apagar Usuário")
	...
	$result10 = mysql_query($query10,$con) or die("Não foi possível apagar suas informações.");
	
			//controle
        $inserir_controle = "INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_acao, a_data, a_hora) VALUES ('".$cod_imob."','".$u_cod."','Excluir Usuário',current_date,current_time)";
   		mysql_query($inserir_controle,$con) or die ("Erro 92 - ".mysql_error());
?>
...
if($B1 == "Atualizar Usuário")
	...
	// AREAS ATUALIZAÇÃO
		...
		$a_result0 = mysql_query($a_query0,$con) or die("Não foi possível inserir suas informações.$a_query0");
		if (sizeof($areas) > 0) {
			foreach($_POST["areas"] as $area_id) {
				$a_query = "INSERT INTO rel_area_usuario (area_id,u_cod, cod_imobiliaria) VALUES('".$area_id."','".$tmp_id."','".$cod_imob."')";
				$a_result = mysql_query($a_query,$con) or die("Não foi possível inserir suas informações.$a_query");
			}
		}
	 // fim valida acesso
	 		...
	 		//controle
        $inserir_controle = "INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_acao, a_data, a_hora) VALUES ('".$cod_imob."','".$u_cod."','".$B1."',current_date,current_time)";
   		mysql_query($inserir_controle,$con) or die ("Erro 132 - ".mysql_error());
...
<?php
	...
	// AREAS ATUALIZAÇÃO
	 ...
	 // fim valida acesso
	 		...
	 		//controle
        $inserir_controle = "INSERT atualizacoes (cod_imobiliaria, a_cod_user, a_acao, a_data, a_hora) VALUES ('".$cod_imob."','".$u_cod."','".$B1."',current_date,current_time)";
   		mysql_query($inserir_controle,$con) or die ("Erro 157 - ".mysql_error());
...
if($lista == "")
	...
	$result1 = mysql_query($query1,$con) or die ("erro 178");
?>
<div align="center">
  <center>
...
<?	
	...
	$result2 = mysql_query($query2,$con) or die ("erro 319");
	  ...
	  <tr>
        <td class="style1" colspan="2">
			<table width="500" border="0" cellpadding="0" cellspacing="0">
				...
				<?
					$temp_array[] = "";
					$busca1 = "SELECT area_id FROM rel_area_usuario WHERE u_cod='$not2[u_cod]'";
                    $result3 = mysql_query($busca1,$con) or die ("erro 418");
                    if(mysql_num_rows($result3) > 0)
						while($linha1 = mysql_fetch_array($result3))
							$temp_array[] = $linha1['area_id'];
					$busca2 = "SELECT area_id, area_nome, area_parametro, area_descricao FROM area ORDER BY area_nome ASC";
                    $result4 = mysql_query($busca2,$con) or die ("erro 423");
					if(mysql_num_rows($result4) > 0){
						$cont = 0;
						while($linha2 = mysql_fetch_array($result4)){
