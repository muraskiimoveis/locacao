...
<?php
	...
	if(($HTTP_SERVER_VARS['SERVER_NAME'] == "rebri.com.br") and ($referencia[2] == "logo.php")){
?>
<br>
<?php
...
if($REQUEST_METHOD=="POST") 
{
	...
	//if (file_exists("$caminho_logo$nome_foto"))
		
		$origem = "$caminho_logo" . $nome_foto;
		//$dest = "/home/httpd/htdocs/adunarbr/img_dest/" . $nome_foto2;
		
	$ImageSize = GetImageSize ($origem);
	$Img_w = $ImageSize[0];
	$Img_h = $ImageSize[1];
	if(($Img_w > 90)  and ($Img_h <= 45)){
		$largura = "90";
	}
	elseif(($Img_w/$Img_h) > 2){
		$largura = "90";
	}
	else
	{
...
criar_thumbnail($origem,$origem,$largura,'','JPEG');
rename($origem.$nome_foto, $origem);
