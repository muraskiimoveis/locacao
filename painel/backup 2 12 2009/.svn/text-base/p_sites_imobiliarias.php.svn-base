...
#   $im_site_institucional = addslashes($im_site_institucional);
   ...
   $at_rs = mysql_query($at_sql) or die ("Erro 1059");
   $imagens = "";
...
<?
      ...
      $resultde = mysql_query($queryde) or die ("Erro 1204");
      $img_temp = explode("||",$not2[im_imagens_institucional]);
      $img_temp = str_replace("|","",$img_temp);
      foreach ($img_temp as $temp) {
         if ($temp <> "") {
            mysql_query("INSERT INTO rebri_fotos_temp SET ft_user = '".$_SESSION['usu_cod']."', ft_foto = '$temp', ft_data = CURRENT_DATE, ft_hora = CURRENT_TIME") or die ("Erro 1202");
         }
      }
?>
    <tr>
...
?>
    ...
    <tr>
      ...
      <td width="80%" class=style2> <input type="text" name="im_img_topo_site" id="im_img_topo_site" size="20" class="campo" value="<?php if($_POST['im_img_topo_site']) { print($_POST['im_img_topo_site']); }else{ print($im_img_topo_site); } ?>">
      	<input type="button" value="Selecionar" class="campo" onClick="window.open('p_img_logo.php?status=t&pasta=<?=$pasta?>', 'janela', 'height=500,width=600,scrollbars=yes,status=no,toolbar=no,menubar=no,location=no');"> Tamanho / Formato obrigatórios (218 x 113) / PNG  <br>
