...
$sql = mysql_query($sql,$con) or die ("erro 68");
...
?>
  ...
  <table border="0" cellspacing="1" width="100%">
    <tr>
      <td width="40%" class=style2>Nome Fantasia:</td>
    ...
    <tr>
      ...
      <td width="73%" class=style2> <input type="text" name="im_razao" size="40" class="campo"></td>
    </tr>
    <tr>
    ...
    <tr>
	 ...
	 <tr>
      ...
      <td width="60%" class=style2><select name="contrato_venda" id="contrato_venda" class="campo">
        ...
        <?php
        	$documentosv = "select dp_cod, dp_nome FROM docs_padrao WHERE (dp_cod='5' OR dp_cod='7') ORDER BY dp_nome ASC";
            $result2 = mysql_query($documentosv,$con) or die ("erro 668");
 			while($linhav = mysql_fetch_array($result2)){
    ...
    <tr>
      <td width="40%" class=style2 valign="top">Contrato para Loca&ccedil;&atilde;o Temporada: </td>
      <td width="60%" class=style2><select name="contrato_locacao" id="contrato_locacao" class="campo">
        <option value="">Selecione um contrato</option>
        <?php
        	$documentosl = "select dp_cod, dp_nome FROM docs_padrao WHERE dp_cod='9' ORDER BY dp_nome ASC";
            $result2 = mysql_query($documentosl,$con) or die ("erro 685");
 			while($linhal = mysql_fetch_array($result2)){
    ...
    <tr>
      <td width="40%" class=style2 valign="top">Contrato para Loca&ccedil;&atilde;o Anual: </td>
      <td width="60%" class=style2><select name="contrato_locacao_anual" id="contrato_locacao_anual" class="campo">
        <option value="">Selecione um contrato</option>
        <?php
        	$documentosla = "select dp_cod, dp_nome FROM docs_padrao WHERE dp_cod='10' ORDER BY dp_nome ASC";
            $result2 = mysql_query($documentosla,$con) or die ("erro 703");
 			while($linhala = mysql_fetch_array($result2)){
 		  	$d_nomela = substr ($linhala[dp_nome], 0, 50);
				if($linhala[dp_cod]==$_POST['contrato_locacao_anual']){
					echo('<option value="'.$linhala[dp_cod].'" title="'.$linhala[dp_nome].'" SELECTED>'.$d_nomela.'...</option>');
				}else{
					echo('<option value="'.$linhala[dp_cod].'" title="'.$linhala[dp_nome].'">'.$d_nomela.'...</option>');
				}
 			}
 	?>
      </select></td>
...
<?php
	...
	$result4 = mysql_query($query4,$con) or die ("erro 759");
	//echo $query4;
