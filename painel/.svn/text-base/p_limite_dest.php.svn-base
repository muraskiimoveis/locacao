...
if($B1 == "Atualizar limite")
	...
	$query = "update rebri_destaques set d_qtd='$qtd' where d_cod='1'";
...
<?php
		...
		$query1 = "select d_qtd from rebri_destaques where d_cod='1'";
		$result1 = mysql_query($query1,$con) or die("Não foi possível atualizar suas informações.");
...
?>
  ...
  <table border="0" cellspacing="1" width="95%">
    <tr>
      <td align="left">Para cada <input class=campo type="text" name="qtd" size="3" value="<?php print("$not1[d_qtd]"); ?>"> imóveis cadastrados as imobiliárias terão direito a 1 imóvel em destaque</td>
    </tr>
    <tr>
